...
class DrawLine128 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     if (clickCount>=0){
        ...
        if (done2){             
           if (aroundPoint(startX2,startY2,xcenterC,ycenterC)){
              g2d.drawString("X柏树", xcenterC, ycenterC); 
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterC,ycenterC);  
              g2d.drawString("60度", xcenterA-50, ycenterA-20);   
           }else if (aroundPoint(startX2,startY2,xcenterD,ycenterD)){
              g2d.drawString("X柏树", xcenterD, ycenterD);  
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterD,ycenterD);  
              g2d.drawString("60度", xcenterA+20, ycenterA-20); 
           }                                  
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请首先画出从A点安全过马路的最短路线,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经画出从A点安全过马路的最短路线,你画了" + (clickCount+1) + "次;"; 
              comment2 = "请继续在马路对面找到柏树对应的点，该柏树与A点连线正好与马路边成60度夹角";
           }else{
        ...
        comment = "图中给出了马路和人行横道，请首先画出从A点安全过马路的最短路线。";
        comment2 = "然后，在马路对面找到柏树对应的点，该柏树与A点连线正好与马路边成60度夹角。";        
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     g2d.drawString("比例尺1:1000", 30+60, 430);  
     g2d.drawString("人", 510-480/4, 160); 
     g2d.drawString("行", 510-120, 180);
     g2d.drawString("横", 510-120, 200); 
     g2d.drawString("道", 510-120, 220);     
