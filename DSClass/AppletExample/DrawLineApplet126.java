...
public class DrawLineApplet126 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine126());
     
   }  
}

class DrawLine126 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterB = 200; //100; // Center position of B
  private static int xcenterC = 100;//0;
  private static int ycenterC = 400;//500; //400; // Center position of C
  private static int xcenterB2 = 300;
  private static int ycenterB2 = 100; //0; // Center position of B
  private static int xcenterC2 = 100;//0;
  private static int ycenterC2 = 300;//400; //300; // Center position of C   
  private static int xcenterO = 150;//100;
  private static int ycenterO = 350;//300;

  private static int xcenterD = 0;
  private static int ycenterD = 350;
  private static int xcenterD2 = 0;
  private static int ycenterD2 = 250;
  private static int xcenterA2 = 50;//100;
  private static int ycenterA2 = 250;//300; 
  private static int xcenterO2 = 40;//100;
  private static int ycenterO2 = 270;//300;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("甲村", xcenterA2-20, ycenterA2-20);
     g2d.fillArc(xcenterA-5, ycenterA-5, 10, 10, 0, 360);
     g2d.drawString("乙村", xcenterA-20, ycenterA-20);
     g2d.drawString("公路", xcenterC2, ycenterC2+20);
     ...
     if (clickCount>=0){
        if (done1 == false){
           if (perpendicular(startX, startY, xcenterA2, ycenterA2, xcenterC2, ycenterC2, xcenterD2, ycenterD2)){
              done1 = true;                               
              comment = "你画对了甲村到公路的垂线，请继续画乙村到公路的垂线,你画了" + (clickCount+1)/2 + "次";  
           }else          
              comment = "你还没有画对,请继续点击画线：画出过甲村到公路的垂线,你画了" + (clickCount+1)/2 + "次;";
        }else{
           if (drawCorrect==false){
              if (perpendicular(startX2, startY2, xcenterA, ycenterA, xcenterB, ycenterB, xcenterC, ycenterC)){
                 done2 = true; 
                 drawCorrect = true;                 
                 comment = "你完全画对了,你画了" + (clickCount+1)/2 + "次;请返回原答题页面点击按钮完成填空，然后提交";  
              }else          
                 comment = "你还没有画对,请继续点击画线：画出过乙村到公路的垂线,你画了" + (clickCount+1)/2 + "次;"; 
           }else
              comment = "你完全画对了,你画了" + (clickCount+1)/2 + "次;请返回原答题页面点击按钮完成填空，然后提交"; 
        }        
     }else{  
        comment = "你还没有开始画，请用鼠标点击画线：画出过甲村到公路的垂线"; 
     }
