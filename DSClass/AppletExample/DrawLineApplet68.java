...
public class DrawLineApplet68 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine68());
     
   }  
}

class DrawLine68 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = xcenterA+10*30;
  private static int ycenterB = ycenterA; // Center position of B
  private static int xcenterC = xcenterB;
  private static int ycenterC = ycenterB+7*30; // Center position of C  
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("足球场的左上方顶点", xcenterA-20, ycenterA-20); 
     /*    
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到足球场的右上方顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了足球场的右上方顶点,请继续在图中找到足球场的右下方顶点,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了足球场的右上方顶点和右下方顶点,请继续在图中找到左下方顶点,你画了" + (clickCount+1) + "次;"; 
              }else{
        ...
        comment = "图中给出了西山小学足球场的平面图和左上方的顶点，请沿着顺时针方向依次找到另外三个顶点，完成这个图。";        
     }
