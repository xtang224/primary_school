...
public class DrawLineApplet92 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine92());
     
   }  
}

class DrawLine92 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = 150;
  private static int ycenterB = 100; // Center position of B
  private static int xcenterC = 200;
  private static int ycenterC = 250; // Center position of C  
  private static int xcenterD = 100;
  private static int ycenterD = 250; // Center position of D
  private static int xcenterO = 190;
  private static int ycenterO = 220; // Center position of O
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请画梯形的底上的高,你画了" + (clickCount+1) + "次;"; 
        }else{
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面并在答案中输入：" + target + "，然后提交";                    
        }              
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示画梯形的底边上的高；";
        comment2 = ""; 
     }
