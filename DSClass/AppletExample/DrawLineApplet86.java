...
public class DrawLineApplet86 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine86());
     
   }  
}

class DrawLine86 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,一个人先向东走5m，记为+5米，再向西走4米，请画出这个人最后的位置，你画了" + (clickCount+1) + "次;";           
        }else{
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";                                                        
        }       
     }else{   
        comment = "图中给出了一个数轴，一个人先向东走5m，记为+5米，再向西走4米，请画出这个人最后的位置;";             
     }
