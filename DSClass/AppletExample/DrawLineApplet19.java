...
public class DrawLineApplet19 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine19());
     
   }  
}

class DrawLine19 extends JPanel implements MouseListener, ActionListener
{
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){           
           ...
           if (Math.abs(startY-ycenterA)<=10 && (startX-xcenterA)>20){
              done1 = true; 
              xcenterB = startX;
              ycenterB = ycenterA;                        
              gp.moveTo(xcenterA,ycenterA);
        ...
        if (done2){
           gp.moveTo(xcenterA,ycenterA);
           gp.lineTo(startX2,startY2);    
           gp.moveTo(xcenterC,ycenterC);
           gp.lineTo(startX2,startY2);   
           gp.moveTo(xcenterC,ycenterC);
           gp.lineTo(xcenterB,ycenterB);            
        }else{
           if (clickCount>=1 && !aroundPoint(startX2, startY2, xcenterA, ycenterA) && startX2<xcenterA && startY2>ycenterA){
              done2 = true;                 
              xcenterC = xcenterB + (xcenterA - startX2); //等腰梯形
              ycenterC = startY2;    
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(startX2,startY2);  
              gp.moveTo(xcenterC,ycenterC);
        ...
        if (done3){
           ...
           //if (clickCount>=2 && inLine(startX2, startY2, xcenterC, ycenterC, startX3, startY3) && perpendicular(startX3, startY3, xcenterA, ycenterA, startX2, startY2, xcenterC, ycenterC)){
           if (clickCount>=2 && aroundPoint(startX3,startY3,xcenterO,ycenterO)){ 
              done3 = true;     
              drawCorrect = true;         
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterO,ycenterO);             
        ...
        if (done3){
           ...
           gp.lineTo(startX3,startY3);   
           gp.moveTo(startX2,startY2);
           gp.lineTo(startX3,startY3);           
        }else{
           if (clickCount>=2 && parallel(startX2, startY2, startX3, startY3, xcenterA, ycenterA, startX, startY) && !aroundPoint(startX3, startY3, xcenterC, ycenterC)){
              done3 = true;                     
              gp.moveTo(startX,startY);
              gp.lineTo(startX3,startY3);   
              gp.moveTo(startX2,startY2);
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请画通过顶点A的梯形的横边,即上底,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画对了通过顶点A的梯形的横边,请继续画通过顶点A的梯形的另外一条边，即腰，你画了" + (clickCount+1) + "次;";
           }else{ 
              if (done3==false){
                 comment = "你已经画对了通过顶点A的整个梯形,请继续画通过顶点A的梯形的高，你画了" + (clickCount+1) + "次;";
              }else{                                              
                 drawCorrect = true;
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面输入：" + target + "，然后提交";                 
              }              
           }        
        }              
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示先画通过顶点A的梯形的横边，即上底；";
        comment2 = "再画过顶点A的梯形的腰，完成这个梯形。"; 
        comment3 = "最后画通过顶点A的梯形的高。"; 
     }
     
        g2d.drawString(comment, 5, 430);
        if (clickCount<0){
           g2d.drawString(comment2, 5, 450);
           g2d.drawString(comment3, 5, 470);
        }
