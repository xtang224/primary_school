...
public class DrawLineApplet75 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine75());
     
   }  
}

class DrawLine75 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int endY4 = 0;

  private static int startX5 = 0;
  private static int startY5 = 0;
  private static int endX5 = 0; 
  private static int endY5 = 0;

  private static int startX6 = 0;
  private static int startY6 = 0;
  private static int endX6 = 0; 
  private static int endY6 = 0; 
  ...
  private static int ycenterB2 = ycenterA2 + 6*30;
  private static int xcenterC2 = xcenterB2 + 6*30;
  private static int ycenterC2 = ycenterB2;
  private static int xcenterD2 = xcenterC2;
  private static int ycenterD2 = ycenterC2 - 6*30;

  private static int xcenterA3 = xcenterA2 + 7*30;
  private static int ycenterA3 = ycenterA2;
  private static int xcenterB3 = xcenterA3;
  private static int ycenterB3 = ycenterA3 + 3*30;
  private static int xcenterC3 = xcenterB3 + 3*30;
  private static int ycenterC3 = ycenterB3;
  private static int xcenterD3 = xcenterC3;
  private static int ycenterD3 = ycenterC3 - 3*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("B", xcenterA2-10, ycenterA2-10); 

     g2d.fillArc(xcenterA3-5, ycenterA3-5, 10, 10, 0, 360);      
     g2d.drawString("C", xcenterA3-10, ycenterA3-10);    
     ...
     */
     if (clickCount>=0){
        ...
        if (done3){             
           ...
           if (aroundPoint(startX3,startY3,xcenterD2,ycenterD2)){
              done3 = true;                                
              gp.moveTo(xcenterC2,ycenterC2);
              gp.lineTo(xcenterD2,ycenterD2);   
              gp.moveTo(xcenterD2,ycenterD2);
        ...
        if (done6){          
           ...
           gp.lineTo(xcenterD3,ycenterD3); 
           gp.moveTo(xcenterD3,ycenterD3);
           gp.lineTo(xcenterA3,ycenterA3);                                 
        }else{
           if (aroundPoint(startX6,startY6,xcenterD3,ycenterD3)){
              done6 = true; 
              drawCorrect = true;                  
              gp.moveTo(xcenterC3,ycenterC3);
              gp.lineTo(xcenterD3,ycenterD3); 
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,要按3：1的比例把给出的正方形放大,已经给出一个顶点，";    
           comment2 = "请按照逆时针方向点击选择和已画出顶点在同一竖直线上的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了3：1放大正方形的一个顶点";
              comment2 = "请继续按照逆时针方向找3：1放大正方形的另外一个顶点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了3：1放大正方形的2个顶点";
                 comment2 = "请继续按照逆时针方向找3：1放大正方形的最后一个顶点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4==false){
                    comment = "你已经找到了3：1放大的正方形B，请接下来继续把正方形B按照1：2的比例缩小为正方形C，";
                    comment2 = "图中已经给出正方形C的一个顶点，请按照逆时针方向找它的另外一个顶点,你画了" + (clickCount+1) + "次;";
                 }else{
                    if (done5==false){
                       comment = "你已经找到了正方形C的一个顶点";
                       comment2 = "请继续按照逆时针方向找正方形C的另外一个顶点,你画了" + (clickCount+1) + "次;";
                    }else{
                       if (done6==false){
                          comment = "你已经找到了正方形C的2个顶点";
                          comment2 = "请继续按照逆时针方向找正方形C的最后一个顶点,你画了" + (clickCount+1) + "次;";
                       }else{
        ...
        comment = "图中给出了正方形，现给出3：1放大的正方形B的一个顶点，请按逆时针方向找到其他3个顶点，完成这个正方形。";        
        comment2 = "然后，再把正方形B按照1：2的比例缩小得到正方形C，图中给出了一个顶点，请按逆时针方向找到其他3个顶点，完成这个正方形。";        
     }
