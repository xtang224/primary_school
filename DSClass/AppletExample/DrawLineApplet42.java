...
class DrawLine42 extends JPanel implements MouseListener, ActionListener
{
  ...
  DrawLine42()
  {
    ...
    if (target.equals("平行线长方形")){
       //do nothing
    }else {// if (target.equals("平行线梯形"))
       //do nothing 
    }
  }
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("直线a", xcenterP1+20, ycenterP1-20);     
     gp.moveTo(xcenterP3,ycenterP3);
     gp.lineTo(xcenterP4,ycenterP4);
     g2d.drawString("直线b", xcenterP3+20, ycenterP3-20);     
     g2d.draw(gp);
     ...
     if (clickCount>=0){
        ...
        if (done1){           
           ...
           if (target.equals("平行线长方形") && aroundPoint(startX, startY, xcenterB,ycenterB)){
              done1 = true;                  
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterB,ycenterB);      
           }else if (target.equals("平行线梯形") && Math.abs(startY-ycenterP3)<=30 && Math.abs(startX-xcenterA)>30){
              done1 = true;
              xcenterB = startX;
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterB,ycenterB);
        ...
        if (done2){           
           ...
           if (target.equals("平行线长方形") && Math.abs(startY2-ycenterA) <= 30 && !aroundPoint(startX2, startY2, xcenterA,ycenterA)){
              done2 = true;    
              xcenterD = startX2;
              ycenterD = ycenterA;
              xcenterC = xcenterD;
              ...
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterD,ycenterD);      
          }else if (target.equals("平行线梯形") && Math.abs(startY2-ycenterA) <= 30 && Math.abs(startX2-xcenterA)>30 && Math.abs(startX2-xcenterB)>30){
              done2 = true;    
              xcenterD = startX2;
              ycenterD = ycenterA;
        ...
        if (done3){           
           ...
           if (target.equals("平行线长方形") && aroundPoint(startX3, startY3, xcenterC,ycenterC)){
              done3 = true;    
              drawCorrect = true;       
              gp.moveTo(xcenterC,ycenterC);
              gp.lineTo(xcenterD,ycenterD);      
              gp.moveTo(xcenterC,ycenterC);
              gp.lineTo(xcenterB,ycenterB);                 
           }else if (target.equals("平行线梯形") && Math.abs(startY3-ycenterB)<=30 && !aroundPoint(startX3, startY3, xcenterO,ycenterO)){
              done3 = true;    
              drawCorrect = true;       
              xcenterC = startX3;
              ycenterC = ycenterB;
     ...
     if (clickCount>=0){
        if (done1==false){  
           if (target.equals("平行线长方形"))
              comment = "你还没有画对,请根据已有的顶点A画出长方形在直线b上的另外一个顶点,你画了" + (clickCount+1) + "次;";            
           else // if (target.equals("平行线梯形"))
              comment = "你还没有画对,请根据已有的顶点A画出梯形在直线b上的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2 == false){
              if (target.equals("平行线长方形"))
                 comment = "你已经找到了长方形在直线b上的另外一个顶点,请继续找长方形在直线a上的另外一个顶点,你画了" + (clickCount+1) + "次;";
             else // if (target.equals("平行线梯形"))
                 comment = "你已经找到了梯形在直线b上的另外一个顶点,请继续找梯形在直线a上的另外一个顶点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3 == false){
                 if (target.equals("平行线长方形"))
                    comment = "你已经找到了长方形的两个顶点,请继续找长方形在直线b上的第4个顶点,你画了" + (clickCount+1) + "次;";
                 else // if (target.equals("平行线梯形"))
                    comment = "你已经找到了长方形的两个顶点,请继续找长方形在直线b上的第4个顶点,你画了" + (clickCount+1) + "次;";            
              }else{
                 drawCorrect = true;
        ...
        if (target.equals("平行线长方形")){      
           comment = "你还没有开始画：请根据已有的长方形的顶点A画出其在直线a和b上的另外3个顶点，完成这个长方形"; 
           comment2 = "请先画出过和点A同边的长方形在直线b上的顶点";
        }else { //if (target.equals("平行线梯形"))
           comment = "你还没有开始画：请根据已有的梯形的顶点A画出其在直线a和b上的另外3个顶点，完成这个梯形"; 
           comment2 = "请先画出过和点A同边的梯形在直线b上的顶点";
        }
     }
