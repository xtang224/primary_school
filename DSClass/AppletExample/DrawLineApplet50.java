...
public class DrawLineApplet50 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine50());
     
   }  
}

class DrawLine50 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){  
           ...
           if (target.equals("分成两个相同三角形") && (aroundPoint(startX,startY,xcenterA,ycenterA) || aroundPoint(startX,startY,xcenterD,ycenterD))){
              done1 = true;
              if (aroundPoint(startX,startY,xcenterA,ycenterA)){
                 ...
                 xcenterO2 = xcenterC;
                 ycenterO2 = ycenterC;
              }else{//aroundPoint(startX,startY,xcenterD,ycenterD)
                 xcenterO = xcenterD;
                 ycenterO = ycenterD;
                 xcenterO2 = xcenterB;
                 ycenterO2 = ycenterB;
              ...
              g2d.fillArc(xcenterO-5, ycenterO-5, 10, 10, 0, 360);   
              //g2d.drawString("O", xcenterO+20, ycenterO-20);
           }else if  (target.equals("分成两个梯形") && Math.abs(startY-ycenterA)<=30 && startX>xcenterA+20 && startX<xcenterD-20){
              done1 = true;              
              xcenterO = startX;
              ycenterO = ycenterA;
        ...
        if (done2){           
           ...
           if (target.equals("分成两个相同三角形") && aroundPoint(startX2,startY2,xcenterO2,ycenterO2)){
              done2 = true;
              drawCorrect = true;                 
              gp.moveTo(xcenterO,ycenterO);              
              gp.lineTo(xcenterO2,ycenterO2);       
           }else if (target.equals("分成两个梯形") && Math.abs(startY2-ycenterB)<=30 && startX2>xcenterB && startX2<xcenterC && !aroundPoint(startX2, startY2, xcenterON, ycenterON)){
              done2 = true;
              drawCorrect = true; 
              xcenterO2 = startX2;
              ycenterO2 = ycenterB;             
     ...
     if (clickCount>=0){
        if (done1==false){  
           ...
           if (done2==false){
              if (target.equals("分成两个相同三角形")){
                 comment = "你已经在过点A的横边上找了一点，请继续在A的对边上找一点并连线。你画了" + (clickCount+1) + "次;";  
                 comment2 = "这条连线会将这个平行四边形分成两个相同的三角形。";
              }else{//(if (target.equals("分成两个梯形"))
                 comment = "你已经在过点A的横边上找了一点，请继续在A的对边上找一点并连线。你画了" + (clickCount+1) + "次;";  
                 comment2 = "这条连线会将这个长方形分成两个梯形。";
              }
           }else{
              drawCorrect = true;
        ...
        if (target.equals("分成两个相同三角形")) 
           comment2 = "再过这点和对边的一点连线将这个平行四边形分成两个相同三角形";  
        else  
           comment2 = "再过这点和对边连线将这个平行四边形分成两个梯形";    
     }
