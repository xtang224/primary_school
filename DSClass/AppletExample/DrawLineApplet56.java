...
public class DrawLineApplet56 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine56());
     
   }  
}

class DrawLine56 extends JPanel implements MouseListener, ActionListener
{
     ...
     if (clickCount>=0){
        if (done1==false){            
           ...
           if (done2==false)
              ...
              if (done3==false)
                 comment = "你已经在点A的左侧、右侧画对,请在点A的下方找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;";
              else
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";
           }            
        }       
     }else{   
        comment = "图中给出了从上面看物体的一个正方形，请依次在点A的左侧、右侧和下方找到相应的顶点，画出其余的图形。";        
     }
