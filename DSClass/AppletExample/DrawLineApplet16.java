...
public class DrawLineApplet16 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine16());
     
   }  
}

class DrawLine16 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请点击边CD上的一点，作通过顶点A并且能够将这个平行四边形分成两个相同三角形的线段,你画了" + (clickCount+1) + "次;"; 
        }else{                                             
           drawCorrect = true;
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交";                               
        }       
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示先画通过顶点A，另一个端点在边CD上的一条线段；";
        comment2 = "这条线段将把这个平行四边形分成两个完全相同的三角形。"; 
     }
