...
public class DrawLineApplet79 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine79());
     
   }  
}

class DrawLine79 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = xcenterA+5*60;
  private static int ycenterB = ycenterA; // Center position of B
  private static int xcenterC = xcenterB;
  private static int ycenterC = ycenterB+2*60; // Center position of C  
  private static int xcenterD = xcenterC-5*60;
     ...
     */
     if (clickCount>=0){
        ...
        if (done2){  
           ...
           gp.lineTo(xcenterA,ycenterA);   
           g2d.setPaint(Color.red);
           g2d.fill(new Rectangle(xcenterA,ycenterA,xcenterC-xcenterA,ycenterC-ycenterA));                     
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中沿着顺时针方向找到长方形的第三个顶点,你画了" + (clickCount+1) + "次;";
           comment2 = ""; 
        }else{
           if (done2==false){
              comment = "你已经找到了长方形的第三个顶点,请继续沿着顺时针方向找到长方形的第四个顶点,你画了" + (clickCount+1) + "次;"; 
              comment2 = "这个长方形涂色后，会占表格总数的40%";
           }else{
        ...
        comment = "图中给出了格子图，并给出了线段AB，请以AB为边，再沿着顺时针方向找到长方形的另外2个顶点;";  
        comment2 = "组成长方形并涂色,使涂颜色部分占格子总数的40%;";       
     }
