...
public class DrawLineApplet112 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine112());
     
   }  
}

class DrawLine112 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA2 = xcenterO - 4*30;
  private static int ycenterA2 = ycenterO;
  private static int xcenterB2 = xcenterA2 + 2*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请点击选择点A按顺时针方向旋转270度后到达的点,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了点A按顺时针方向旋转270度后到达的点";
              comment2 = "请继续找点B按顺时针方向旋转270度后到达的点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了点A，B按顺时针方向旋转270度后到达的点";
                 comment2 = "请继续找点C按顺时针方向旋转270度后到达的点,你画了" + (clickCount+1) + "次;";
              }else{
        ...
        comment = "图中给出了三角形小旗，请完成这个小旗按顺时针方向旋转270度后的图形。";   
       comment2 = "请点击选择点A按顺时针方向旋转270度后到达的点。";     
     }
