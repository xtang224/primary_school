...
public class DrawLineApplet58 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine58());
     
   }  
}

class DrawLine58 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterO = xcenterA;
  private static int ycenterO = ycenterA-80; // Center position of O
  private static int xcenterO2 = xcenterD;
  private static int ycenterO2 = ycenterD-80; // Center position of O2
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在点A的上方找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false)
              comment = "你已经在点A的上方画对,请在点A的右侧找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;";
           else{
              comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";
           }            
        }       
     }else{   
        comment = "图中给出了从左面看物体的一个正方形，请依次在点A的上方和右侧找到相应的顶点，画出其余的图形。";        
     }
