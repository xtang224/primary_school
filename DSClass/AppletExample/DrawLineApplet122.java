...
public class DrawLineApplet122 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine122());
     
   }  
}

class DrawLine122 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 360;
  ...
  private static int xcenterD = (xcenterA+xcenterC)/2;
  private static int ycenterD = (ycenterA+ycenterC)/2; // Center position of D
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请你找到原来的第三个顶点，画出完整的三角形,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
        ...
        comment = "图中给出了破损的直角三角形，请你找到原来的第三个顶点，画出完整的三角形。";        
     }
