...
public class DrawLineApplet54 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine54());
     
   }  
}

class DrawLine54 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterH = xcenterA;
  private static int ycenterH = ycenterA-80;
  private static int xcenterH2 = xcenterD;
  private static int ycenterH2 = ycenterD-80;
  private static int xcenterL = xcenterB;
  private static int ycenterL = ycenterB+80;
  private static int xcenterL2 = xcenterC;
  private static int ycenterL2 = ycenterC+80;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done3){  
           ...
           gp.moveTo(xcenterL,ycenterL);
           gp.lineTo(xcenterL2,ycenterL2);  
           gp.moveTo(xcenterL2,ycenterL2);
           gp.lineTo(xcenterC,ycenterC);           
        }else{
           if (aroundPoint(startX3,startY3,xcenterL,ycenterL) || aroundPoint(startX3,startY3,xcenterL2,ycenterL2)){
              done3 = true; 
              drawCorrect = true; 
              gp.moveTo(xcenterB,ycenterB);
              gp.lineTo(xcenterL,ycenterL); 
     ...
     if (clickCount>=0){
        if (done1==false){            
           ...
           if (done2==false)
              comment = "你已经在点A的左侧画对,请在点A的上方找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;";
           else{
              if (done3==false)
                 comment = "你已经在点A的左侧、上方画对,请在点A的下方找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;";
              else
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";
           }            
        }       
     }else{   
        comment = "图中给出了从上面看物体的一个正方形，请依次在点A的左侧、上方和下方找到相应的顶点，画出其余的图形。";        
     }
