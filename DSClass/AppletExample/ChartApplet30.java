...
public class ChartApplet30 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new ChartLine30());
     
   }  
}

class ChartLine30 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA = 100 + 30; // Center position of A
  private static int xcenterB = 190;
  private static int ycenterB = 100 + 8*30; // Center position of B
  private static int xcenterC = 250;
  private static int ycenterC = 100 + 3*30; // Center position of C  
  private static int xcenterD = 310;
  private static int ycenterD = 100 + 2*30; // Center position of D
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        if (done1){  
           g2d.fillRect(xcenterA, ycenterA, 30, 11*30);              
        }else{
        ...
        if (done2){
           g2d.fillRect(xcenterB, ycenterB, 30, 4*30);            
        }else{
        ...
        if (done3){
           g2d.fillRect(xcenterC, ycenterC, 30, 9*30);         
        }else{
        ...
        if (done4){
           g2d.fillRect(xcenterD, ycenterD, 30, 10*30);         
        }else{
        ...
        if (done5){
           g2d.fillRect(xcenterE, ycenterE, 30, 8*30);         
        }else{
        ...
        if (done6){
           g2d.fillRect(xcenterF, ycenterF, 30, 36+10*30);         
        }else{
        ...
        if (done7){
           g2d.fillRect(xcenterG, ycenterG, 30, 10*30);         
        }else{
           if (between2Points(startX7, startY7, xcenterG,ycenterG,xcenterG+30,ycenterG)){
              done7 = true;   
              drawCorrect = true;            
              g2d.fillRect(xcenterG, ycenterG, 30, 10*30);         
           }           
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请根据要求先画出95分整数和小数的柱状图,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画出了整数和小数的柱状图，";
              comment2 = "请继续画60分分数和百分数的柱状图。你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经画出了整数和小数、分数和百分数的柱状图，";
                 comment2 = "请继续画85分量与计算的柱状图。你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经画出了整数和小数、分数和百分数、量与计算的柱状图，";
                    comment2 = "请继续画90分代数知识的柱状图。你画了" + (clickCount+1) + "次;";
                 }else{
                    if (done5 == false){
                       comment = "你已经画出了整数和小数、分数和百分数、量与计算、代数知识的柱状图，";
                       comment2 = "请继续画80分几何知识的柱状图。你画了" + (clickCount+1) + "次;";
                    }else{
                       if (done6 == false){
                          comment = "你已经画出了整数和小数、分数和百分数、量与计算、代数知识、几何知识的柱状图，";
                          comment2 = "请继续画96分比和比例的柱状图。你画了" + (clickCount+1) + "次;";
                       }else{
                          if (done7 == false){
                             comment = "你已经画出了整数和小数、分数和百分数、量与计算、代数知识、几何知识、比和比例的柱状图，";
                             comment2 = "请继续画90分统计知识的柱状图。你画了" + (clickCount+1) + "次;";
                          }else{                              
        ...
        comment = "请按照提示先后画整数和小数、分数和百分数、量与计算、代数知识、几何知识、比和比例、统计知识的柱状图。";
        comment2 = "即画出95分整数和小数、60分分数和百分数、85分量与计算、90分代数知识、80分几何知识、96分比和比例以及90分统计知识的条形统计图。"; 
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","整数和","分数和","量与","代数","几何","比和","统计"};
  ...
  public void drawRuler2(Graphics2D g2d){
     ...
     for (int i=0; i<=120; i++){        
        if (i % 20 == 0){
           intV = i/20;
           str = (intV*10+40) + "";
           g2d.drawString(str, 100-20, 460-i*3);
        }
     }      
     gp.moveTo(100,460);
     gp.lineTo(100,60);
