...
public class DrawLineApplet extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine());     
   }  
}

class DrawLine extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     int targetX = (int) (Math.cos(intTarget * Math.PI / 180) * 200 + startX);
     int targetY = (int) (startY - Math.sin(intTarget * Math.PI / 180 ) * 200);
     double targetAngle = (double)(targetX - startX) / (targetY - startY); //here, the calculation of angle is actually dx/dy, whichh is the inverse of normal math definition
     double actualAngle = (double)(endX - startX) / (endY - startY);
     if (firstClick){
        if ((targetAngle >= actualAngle-0.02) && (targetAngle <= actualAngle+0.02)){
     ...
     g2d.drawString(comment, startX-400, startY+50);
