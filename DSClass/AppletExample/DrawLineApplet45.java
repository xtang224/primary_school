...
public class DrawLineApplet45 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine45());
     
   }  
}

class DrawLine45 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done2){           
           ...
           if (parallel(startX2, startY2, xcenterB, ycenterB, xcenterA, ycenterA, xcenterC, ycenterC)){
              done2 = true;
              drawCorrect = true;                   
              gp.moveTo(xcenterB,ycenterB);
              gp.lineTo(xcenterO2,ycenterO2);      
     ...
     if (clickCount>=0){
        if (done1==false){  
           ...
           if (done2==false){
              comment = "你已经过点A画了BC的垂线,请再过点B画AC的平行线。你画了" + (clickCount+1) + "次;";  
           }else{
              drawCorrect = true;
        ...
        comment = "你还没有开始画：请先过点A画BC的垂线"; 
        comment2 = "再过点B画AC的平行线";        
     }
