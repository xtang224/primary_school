...
public class DrawLineApplet77 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine77());
     
   }  
}

class DrawLine77 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA = 530; // Center position of A
  private static int xcenterB = xcenterA+2*30;
  private static int ycenterB = ycenterA-2*30; // Center position of B
  private static int xcenterC = xcenterA+4*30;
  private static int ycenterC = ycenterA-4*30; // Center position of C  
  private static int xcenterD = xcenterA+13*30;
  private static int ycenterD = ycenterA-13*30; // Center position of D
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("", xcenterA-20, ycenterA-20); 
     /*    
     ...
     */ 
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到1小时行驶80km的坐标位置,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了1小时的位置,请继续在图中找到2小时行驶160km的位置,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了1小时和2小时的位置,请继续在图中找到6.5小时行驶520km的位置，你画了" + (clickCount+1) + "次;"; 
              }else{
        ...
        comment = "图中给出了路程和时间的坐标图，请分别标出1小时、2小时及6.5小时的时间和路程的坐标。";        
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     for (int i=0; i<=70; i++){        
        if (i % 10 == 0){
           intV = i/10;           
           g2d.drawString(strs[intV], 30+i*6, 550);
     ...
     gp.moveTo(510,530);
     gp.lineTo(500,540);   
     g2d.drawString("时间/时", 500, 530);   
     g2d.drawString("", 230+60, 550);  
  ...
  public void drawRuler2(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","","80","","160","","240","","320","","400","","480","","","",""};
     //String str;
     int intV = 0;
     for (int i=0; i<=160; i++){        
        if (i % 10 == 0){
           intV = i/10;
           //str = intV*50 + "";
           g2d.drawString(strs[intV], 5, 530-i*3);           
        }
     }      
     gp.moveTo(30,530);
     gp.lineTo(30,20);
