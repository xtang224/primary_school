...
public class DrawLineApplet85 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine85());
     
   }  
}

class DrawLine85 extends JPanel implements MouseListener, ActionListener
{
     ...
     */
     if (clickCount>=0){
        ...
        if (done2){  
           ...
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);      
           g2d.drawString("D", xcenterD+20, ycenterD+20);
           g2d.fillRect(xcenterA, ycenterA, 180, 180);                   
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中沿着逆时针方向找到顶点C，使长方形ABCD的面积占整个面积的75%,你画了" + (clickCount+1) + "次;";           
        }else{
           if (done2==false){
              comment = "你已经找到了C点，请继续沿着逆时针方向找D点,使长方形ABCD的面积占整个面积的75%,你画了" + (clickCount+1) + "次;";               
           }else{
              comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";                                              
           }          
        }       
     }else{   
        comment = "图中给出了方框和线段AB，请沿着逆时针方向找到另外两个顶点C和D，使长方形ABCD的面积占整个图形的75%;";             
     }
