...
public class DrawLineApplet66 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine66());
     
   }  
}

class DrawLine66 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 240;
  private static int ycenterA = 500; // Center position of A
  private static int xcenterB = xcenterA-5*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("小明家", xcenterA-20, ycenterA-20); 
     /*    
     ...
     */
     ...
     if (clickCount>=0){
        ...
        if (done1){  
           g2d.fillArc(xcenterB-5, ycenterB-5, 10, 10, 0, 360);
           g2d.drawString("街心公园",xcenterB-20, ycenterB-20);                       
        }else{
        ...
        if (done2){  
           g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);
           g2d.drawString("科技馆",xcenterC-20, ycenterC-20);                       
        }else{
        ...
        if (done3){  
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);
           g2d.drawString("动物园",xcenterD-20, ycenterD-20);                       
        }else{
        ...
        if (done4){  
           g2d.fillArc(xcenterO-5, ycenterO-5, 10, 10, 0, 360);
           g2d.drawString("医院",xcenterD-20, ycenterD-20);                       
        }else{
           if (aroundPoint(startX4,startY4,xcenterO,ycenterO)){
              done4 = true; 
              drawCorrect = true; 
              g2d.fillArc(xcenterO-5, ycenterO-5, 10, 10, 0, 360);
              g2d.drawString("医院",xcenterO-20, ycenterO-20);                             
           }           
        }      
     }
     g2d.draw(gp);
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到街心公园，位于小明家正西方向500m,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了街心公园的位置,请继续在图中找到科技馆的位置，位于街心公园正北方向300m,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了街心公园和科技馆位置,请继续在图中找到动物园的位置，位于科技馆正东方向1km,你画了" + (clickCount+1) + "次;"; 
              }else{
                 if (done3==false){
                    comment = "你已经找到了街心公园,科技馆和动物园位置,请继续在图中找到医院的位置，位于动物园正南方向400m,你画了" + (clickCount+1) + "次;"; 
                 }else
        ...
        comment = "图中给出了小明家的平面图，请分别找到街心公园、科技馆、动物园、医院的位置。";        
     }
