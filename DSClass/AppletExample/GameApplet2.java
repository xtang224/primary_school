...
{  
   ...
   {  
      ...
      contentPane.add(new Game2Panel());
   }
}

class Game2Panel extends JPanel
...
{  
    ...
    private String problemHint = "你和对手轮流报数，每次只能报1或2，把两人报的所有数加起来，谁报数后和是25，谁就获胜";
       ...
       if (command.equals("点击此按钮提交你的对策")){
          ...
          for (int i=0; i<maxGameSize; i++){
             ...
             if (tmpInteger.intValue()!=0){
                if (tmpInteger.intValue()<1 || tmpInteger.intValue()>2){
                   label2.setText("你只能填1或2，请重新提交你的对策");
                   return;
                }
                label2.setText("你提交的对策是1或2，符合要求");
                yourInput += tmpInteger + ";";
             }
          }
          System.out.println("before setYourInput(), yourInput =" + yourInput); 
          setYourInput();
          ...
          if (gameSum < gameCritical){
             ...
             label2.setText("游戏结束，所有的数加起来的和不能超过25。请点击最下面一个按钮完成这道题。");
             done = true;
             win = false;
             myTable.setValueAt(new String("对手"),3,3);    
             return; 
