...
public class DrawLineApplet132 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine132());
     
   }  
}

class DrawLine132 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB2 = xcenterA2 + 3*30;
  private static int ycenterB2 = ycenterA2;
  private static int xcenterC2 = xcenterB2;
  private static int ycenterC2 = ycenterB2 + 2*30;
  private static int xcenterD2 = xcenterC2 - 3*30;
  private static int ycenterD2 = ycenterC2;
     ...
     */
     if (clickCount>=0){
        ...
        if (done4){           
           ...
           if (aroundPoint(startX4,startY4,xcenterB2,ycenterB2)){
              done4 = true;    
              gp.moveTo(xcenterA2,ycenterA2);
              gp.lineTo(xcenterB2,ycenterB2);                        
           }           
        ...
        if (done5){           
           ...
           if (aroundPoint(startX5,startY5,xcenterC2,ycenterC2)){
              done5 = true;    
              gp.moveTo(xcenterB2,ycenterB2);
              gp.lineTo(xcenterC2,ycenterC2);                        
           }           
        ...
        if (done6){           
           ...
           if (aroundPoint(startX6,startY6,xcenterD2,ycenterD2)){
              done6 = true;    
              drawCorrect = true;            
              gp.moveTo(xcenterC2,ycenterC2);
              gp.lineTo(xcenterD2,ycenterD2); 
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请找到和点A在同一条水平线上长是6cm的一个顶点B,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了点A在同一条水平线上长是6cm的一个顶点B";
              comment2 = "请继续找和B在同一条竖直线上宽是4cm的另外一个顶点C,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了这个长方形的两个顶点B和C，";
                 comment2 = "请继续找和C在同一水平线上的最后一个顶点D,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经找到了以A为顶点的长方形,请以A2为顶点，画出缩小2倍的长方形";
                    comment2 = "请找到和A2在同一条水平线上长是3cm的顶点B2，你画了" + (clickCount+1) + "次;";                  
                 }else{
                    if (done5 == false){
                       comment = "你已经找到了和A2在同一条水平线上长是3cm的顶点B2";
                       comment2 = "请找到和B2在同一条竖直线上宽是2cm的顶点C2，你画了" + (clickCount+1) + "次;";                  
                    }else{
                       if (done6 == false){
                          comment = "你已经找到了和B2在同一条竖直线上宽是2cm的顶点C2";
                          comment2 = "请找到和C2在同一条水平线上长是3cm的顶点D2，你画了" + (clickCount+1) + "次;";                  
                       }else{
        ...
        comment = "图中每个小方格的边长是1厘米，并给出了点A和A2，请以点A为一个顶点";
        comment2 = "画出一个周长是20cm，长和宽是3：2的长方形，再画出1：2缩小后的长方形。";        
     }
