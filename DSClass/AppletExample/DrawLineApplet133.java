...
public class DrawLineApplet133 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine133());
     
   }  
}

class DrawLine133 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA2 = xcenterA + 8*30;
  private static int ycenterA2 = ycenterA;
  private static int xcenterB2 = xcenterB + 8*30;
  private static int ycenterB2 = ycenterB;
  private static int xcenterC2 = xcenterC + 8*30;
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){             
           ...
           if (aroundPoint(startX,startY,xcenterO2,ycenterO2)){
              done1 = true;              
              g2d.fillArc(xcenterO2-5, ycenterO2-5, 10, 10, 0, 360);                                                            
           }           
        } 
        ...
        if (done2){             
           ...
           if (aroundPoint(startX2,startY2,xcenterA2,ycenterA2)){
              done2 = true;              
              gp.moveTo(xcenterA2,ycenterA2);
              gp.lineTo(xcenterO2,ycenterO2);                             
           }           
        ...
        if (done3){             
           ...
           if (aroundPoint(startX3,startY3,xcenterB2,ycenterB2)){
              done3 = true;                                        
              gp.moveTo(xcenterB2,ycenterB2);                
              gp.lineTo(xcenterA2,ycenterA2);              
           }           
        ...
        if (done4){             
           ...
           if (aroundPoint(startX4,startY4,xcenterC2,ycenterC2)){
              done4 = true; 
              drawCorrect = true;                          
              gp.moveTo(xcenterB2,ycenterB2);
              gp.lineTo(xcenterC2,ycenterC2);                                        
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请点击选择点O向右平移8格后到达的点,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了点O向右平移8格后到达的点";
              comment2 = "请继续找点A向右平移8格后到达的点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了点O，A向右平移8格后到达的点";
                 comment2 = "请继续找点B向右平移8格后到达的点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4==false){
                    comment = "你已经找到了点O，A，B向右平移8格后到达的点";
                    comment2 = "请继续找点C向右平移8格后到达的点,你画了" + (clickCount+1) + "次;";
                 }else{
        ...
        comment = "图中给出了三角形小旗，请完成这个小旗向右平移8格后得到的图形。";   
       comment2 = "请点击选择点O向右平移8格后到达的点。";     
     }
