...
class DrawLine135 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请找到顶点A向右平移5格后的点,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了顶点A向右平移5格后的点，";
              comment2 = "请继续找顶点B向右平移5格后的点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了顶点A、B向右平移5格后的点，";
                 comment2 = "请继续找顶点C向右平移5格后的点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经把三角形ABC向右平移了5格,请继续以A3为顶点将三角形ABC按1：2缩小";
                    comment2 = "请找到缩小的三角形中顶点B对应的点，你画了" + (clickCount+1) + "次;";                  
                 }else{
                    if (done5 == false){
                       comment = "你已经把三角形ABC向右平移了5格,并找到了将三角形ABC按1：2缩小后顶点B所对应的点";
                       comment2 = "请继续找到缩小的三角形中顶点C对应的点，你画了" + (clickCount+1) + "次;";                  
                    }else{
        ...
        comment = "图中给出了三角形ABC，请首先将它向右平移5格"; 
        comment2 = "然后，以A3为顶点，画出三角形ABC按1：2缩小后的图形";       
     }
