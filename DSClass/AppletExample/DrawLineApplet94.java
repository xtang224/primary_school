...
public class DrawLineApplet94 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine94());
     
   }  
}

class DrawLine94 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterC2 = xcenterB2 + 5*30;
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){             
           ...
           if (aroundPoint(startX,startY,xcenterC2,ycenterC2)){
              done1 = true; 
              drawCorrect = true;                  
              gp.moveTo(xcenterB2,ycenterB2);
              gp.lineTo(xcenterC2,ycenterC2);
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,图中给出了以A2为顶点的三角形的一条边，请找到第3个顶点，完成这个三角形；";    
           comment2 = "使这个三角形的面积等于给出的平行四边形的面积,你画了" + (clickCount+1) + "次;"; 
        }else{
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交"; 
           comment2 = "";                                                                                  
        }       
     }else{   
        comment = "图中给出了一个平行四边形，和以A2为起点的三角形的一条边，请找到第3个顶点完成这个三角形，使其面积等于这个平行四边形的面积。";        
     }
