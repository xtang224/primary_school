...
public class DrawLineApplet124 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine124());
     
   }  
}

class DrawLine124 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA2 = ycenterA+6*30;
  private static int xcenterB2 = xcenterA2 + 6*30;
  private static int ycenterB2 = ycenterA2 - 4*30;
  private static int xcenterC2 = xcenterA2 + 8*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "请以A2为顶点完成按2：1比例放大的三角形，请找到在放大的三角形中B所对应的点,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了按2：1比例放大的三角形中B所对应的点";
              comment2 = "请继续找点C在按2：1比例放大的三角形中所对应的点,你画了" + (clickCount+1) + "次;";
           }else{
        ...
        comment = "图中给出了三角形ABC和顶点A2，请以A2为顶点完成按2：1比例放大的三角形。";   
       comment2 = "";     
     }
