...
public class DrawLineApplet82 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine82());
     
   }  
}

class DrawLine82 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 40+23*20;
  private static int ycenterA = 150; // Center position of A
  private static int xcenterB = 40+2*20;
  private static int ycenterB = ycenterA; // Center position of B
  private static int xcenterC = 40+12*20;
  private static int ycenterC = ycenterA; // Center position of C  
  private static int xcenterD = 40+7*20;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){  
           g2d.fillArc(xcenterA-5, ycenterA-5, 10, 10, 0, 360);            
           g2d.drawString("11/2", xcenterA-5, ycenterA-10);                       
        }else{
        ...
        if (done2){  
           g2d.fillArc(xcenterB-5, ycenterB-5, 10, 10, 0, 360);
           g2d.drawString("-5", xcenterB-5, ycenterB-10);                   
        }else{
        ...
        if (done3){  
           g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);
           g2d.drawString("0", xcenterC-5, ycenterC+10);                   
        }else{
        ...
        if (done4){  
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);
           g2d.drawString("-5/2", xcenterD-5, ycenterD-10);                   
        }else{
        ...
        if (done5){  
           ...
           g2d.drawString("-1", xcenterE-5, ycenterE-10);                   
        }else{
        ...
        if (done6){  
           ...
           g2d.drawString("+2", xcenterF-5, ycenterF-10);                   
        }else{
        ...
        if (done7){  
           ...
           g2d.drawString("3.5", xcenterG-5, ycenterG-10);                   
        }else{
        ...
        if (done8){  
           ...
           g2d.drawString("-2", xcenterH-5, ycenterH+20);                   
        }else{
          if (aroundPoint(startX8,startY8,xcenterH,ycenterG)){
              done8 = true;  
              drawCorrect = true;              
              g2d.fillArc(xcenterH-5, ycenterH-5, 10, 10, 0, 360);
              g2d.drawString("-2", xcenterH-5, ycenterH+20);                               
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在数轴上找到11/2,你画了" + (clickCount+1) + "次;";           
        }else{
           if (done2==false){
              comment = "你已经找到了11/2,请继续在数轴上找-5,你画了" + (clickCount+1) + "次;";               
           }else{
              if (done3==false){
                 comment = "你已经找到了11/2，-5,请继续在数轴上找0,你画了" + (clickCount+1) + "次;"; 
              }else{
                 if (done4==false){
                    comment = "你已经找到了11/2，-5,0,请继续在数轴上找-5/2,你画了" + (clickCount+1) + "次;"; 
                 }else{  
                    if (done5==false){
                       comment = "你已经找到了11/2，-5,0,-5/2,请继续在数轴上找-1,你画了" + (clickCount+1) + "次;"; 
                    }else{ 
                       if (done6==false){
                          comment = "你已经找到了11/2，-5,0,-5/2,-1,请继续在数轴上找+2,你画了" + (clickCount+1) + "次;"; 
                       }else{ 
                          if (done7==false){
                             comment = "你已经找到了11/2，-5,0,-5/2,-1,+2,请继续在数轴上找3.5,你画了" + (clickCount+1) + "次;"; 
                          }else{ 
                             if (done8==false){
                                comment = "你已经找到了11/2，-5,0,-5/2,-1,+2,3.5,请继续在数轴上找-2,你画了" + (clickCount+1) + "次;"; 
                             }else
        ...
        comment = "图中给出了数轴，请在数轴上依次标出11/2，-5，0，-5/2，-1, +2, 3.5，-2;";             
     }
