...
public class DrawLineApplet130 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine130());
     
   }  
}

class DrawLine130 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = xcenterO+3*30;
  private static int ycenterA = ycenterO-2*30; // Center position of A
  private static int xcenterB = xcenterO+5*30;
  private static int ycenterB = ycenterO-4*30; // Center position of B
  private static int xcenterC = xcenterO+3*30;
  private static int ycenterC = ycenterO-6*30; // Center position of C  
  private static int xcenterD = xcenterO+1*30;
  private static int ycenterD = ycenterO-4*30; // Center position of D 
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,A位于（3，2），请点击选择A所对应的点。你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了A所对应的点。B在（5，4），";
              comment2 = "请继续找B所对应的点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了A，B所对应的点，C位于（3，6）";
                 comment2 = "请继续找C所对应的点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4==false){
                    comment = "你已经找到了A，B，C所对应的点，D位于（1，4）。";
                    comment2 = "请继续找D所对应的点,你画了" + (clickCount+1) + "次;";
                 }else{
        ...
        comment = "如图，在图上标出A，B，C，D的位置并连线。";   
        comment2 = "点A位于（3，2），请点击选择点A所在的位置。";     
     }
