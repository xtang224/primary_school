...
public class DrawLineApplet80 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine80());
     
   }  
}

class DrawLine80 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 30+5*60;
  private static int ycenterA = 50; // Center position of A
  private static int xcenterB = xcenterA;
  private static int ycenterB = ycenterA+5*60; // Center position of B
  private static int xcenterC = xcenterB+3*60;
  private static int ycenterC = ycenterB; // Center position of C  
  private static int xcenterD = xcenterC;
  private static int ycenterD = ycenterC-5*60; // Center position of D
     ...
     */
     if (clickCount>=0){
        ...
        if (done2){  
           ...
           gp.moveTo(xcenterD,ycenterD);
           gp.lineTo(xcenterA,ycenterA);            
           drawGrid2(g2d,xcenterA,ycenterA,xcenterC,ycenterC);                     
        }else{
           if (aroundPoint(startX2,startY2,xcenterD,ycenterD)){
              done2 = true;  
              drawCorrect = true;              
              gp.moveTo(xcenterC,ycenterC);
              gp.lineTo(xcenterD,ycenterD); 
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中沿着逆时针方向找到长方形的第三个顶点,你画了" + (clickCount+1) + "次;";
           comment2 = ""; 
        }else{
           if (done2==false){
              comment = "你已经找到了长方形的第三个顶点,请继续沿着逆时针方向找到长方形的第四个顶点,你画了" + (clickCount+1) + "次;"; 
              comment2 = "组成长方形并画出空白格子,使原涂颜色部分占格子总数的25%。";
           }else{
        ...
        comment = "图中给出了格子图以及涂色部分，并给出了线段AB，请以AB为边，再沿着逆时针方向向右找到长方形的另外2个顶点;";  
        comment2 = "组成长方形并画出空白格子,使原涂颜色部分占格子总数的25%。";       
     }
