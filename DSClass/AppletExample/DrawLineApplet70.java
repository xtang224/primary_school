...
public class DrawLineApplet70 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine70());
     
   }  
}

class DrawLine70 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterC = xcenterB+3*30;
  ...
  private static int xcenterA2 = xcenterA + 4*30;
  private static int ycenterA2 = ycenterA - 30;
  private static int xcenterB2 = xcenterA2;
  private static int ycenterB2 = ycenterA2 + 4*30;
  private static int xcenterC2 = xcenterB2 + 12*30;
     ...
     */
     if (clickCount>=0){
        ...
        if (done3){           
           ...
           if (aroundPoint(startX3,startY3,xcenterB3,ycenterB3)){
              done3 = true;              
              gp.moveTo(xcenterA3,ycenterA3);
              gp.lineTo(xcenterB3,ycenterB3);                        
           }           
        ...
        if (done4){           
           ...
           if (aroundPoint(startX4,startY4,xcenterC3,ycenterC3)){
              done4 = true;    
              drawCorrect = true;            
              gp.moveTo(xcenterB3,ycenterB3);
              gp.lineTo(xcenterC3,ycenterC3);  
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请按照1：4的比例把三角形A放大为B，请点击选择和已画出顶点在同一竖直线上的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了1：4放大三角形的一个顶点";
              comment2 = "请继续找图中1：4放大三角形的另外一个顶点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了1：4放大的三角形B,请继续把三角形B按2：1的比例缩小，";
                 comment2 = "请点击选择和三角形C已画出顶点在同一竖直线上的另外一个顶点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经找到了1：4放大的三角形B和2:1缩小B的三角形C的一个顶点,";
                    comment2 = "请继续把三角形C的另外一个顶点找到，你画了" + (clickCount+1) + "次;";                  
                 }else{
        ...
        comment = "图中给出了三角形A和其等比例扩大或缩小的另外2个三角形B和C的顶点，请完成另外2个三角形。";        
     }
