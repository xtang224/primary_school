...
public class DrawLineApplet76 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine76());
     
   }  
}

class DrawLine76 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterC = xcenterB+6*30;
  private static int ycenterC = ycenterB; // Center position of C  
  private static int xcenterD = xcenterA - 3*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("小强家", xcenterA-20, ycenterA-20); 
     /*    
     ...
     */
     ...
     if (clickCount>=0){
        ...
        if (done3){  
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);
           g2d.drawString("小伟家",xcenterD-20, ycenterD-20);                       
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到汽车站，位于小强家正北方向400m,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了汽车站的位置,请继续在图中找到学校的位置，位于汽车站正东方向600m,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了汽车站和学校的位置,请继续在图中找到小伟家的位置，位于小强家正西方向300m,你画了" + (clickCount+1) + "次;"; 
              }else{
        ...
        comment = "图中给出了小强家的平面图，请分别找到汽车站、学校、小伟家的位置。";        
     }
