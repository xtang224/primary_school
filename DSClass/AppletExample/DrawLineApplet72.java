...
public class DrawLineApplet72 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine72());
     
   }  
}

class DrawLine72 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterB2 = ycenterA2 + 3*30;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,要按1：3的比例把给出的长方形放大,已经给出一个顶点，";    
           comment2 = "请按照逆时针方向点击选择和已画出顶点在同一竖直线上的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了1：3放大长方形形的一个顶点";
              comment2 = "请继续按照逆时针方向找1：3放大长方形的另外一个顶点,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经找到了1：3放大的长方形的2个顶点，";
                 comment2 = "请继续按照逆时针方向找1：3放大长方形的最后一个顶点,你画了" + (clickCount+1) + "次;";
              }else{
        ...
        comment = "图中给出了长方形，现给出3：1放大的长方形的左上顶点，请按逆时针方向找到其他3个顶点，完成这个长方形。";        
     }
