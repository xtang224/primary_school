...
public class DrawLineApplet91 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine91());
     
   }  
}

class DrawLine91 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 150;
  private static int ycenterA = 100; // Center position of A
  private static int xcenterB = 350;
  ...
  private static int xcenterO = 150;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.draw(gp);
     g2d.drawString("底", (xcenterC+xcenterD)/2, ycenterC+20);
     /*
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请画平行四边形的底上的高,你画了" + (clickCount+1) + "次;"; 
        }else{
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面并在答案中输入：" + target + "，然后提交";                    
        }              
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示画平行四边形的底边上的高；";
        comment2 = ""; 
     }
