...
public class DrawLineApplet104 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine104());
     
   }  
}

class DrawLine104 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请找到点B绕点A旋转90度后的位置,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){
              comment = "你已经找到了点B绕点A旋转90度后的位置，";
              comment2 = "请继续找绕点C旋转90度后的位置,你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经画出了绕点A旋转后形成的三角形，";
                 comment2 = "请点击选择点A向右平移10厘米后形成的点,你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经找到了点A向右平移10厘米后形成的顶点,";
                    comment2 = "请继续找点B绕点A旋转90度后再平移10厘米形成的顶点，你画了" + (clickCount+1) + "次;";                  
                 }else{
                    if (done5 == false){
                       comment = "你已经找到了点B向右平移10厘米后形成的顶点,";
                       comment2 = "请继续找点C绕点A旋转90度后再平移10厘米形成的顶点，你画了" + (clickCount+1) + "次;";                  
                    }else{
        ...
        comment = "图中给出了三角形A，请将这个三角形绕A顺时针旋转90度，画出旋转后的三角形。";        
        comment2 = "再把旋转后的三角形向右平移10厘米，画出平移后的三角形。";        
     }
