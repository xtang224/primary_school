...
public class DrawLineApplet64 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine64());
     
   }  
}

class DrawLine64 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 380;
  private static int ycenterA = 50; // Center position of A
  private static int xcenterB = xcenterA;
  private static int ycenterB = ycenterA+160; // Center position of B
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到最大的质量和总价组合的另外一个点,你画了" + (clickCount+1) + "次;"; 
        }else{           
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";                     
        }       
     }else{   
        comment = "图中给出了总价和质量的图，请画出它们的关系线。";        
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","1","2","3","4","5","6","7","","",""};
     ...
     for (int i=0; i<=70; i++){        
        if (i % 10 == 0){
           intV = i/10;           
           g2d.drawString(strs[intV], 140+i*3, 550);
  ...
  public void drawRuler2(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"0","","2","","4","","6","","8","","10","","12","","14","",""};
     //String str;
     int intV = 0;
     for (int i=0; i<=160; i++){        
        if (i % 10 == 0){
           intV = i/10;
           //str = intV*50 + "";
           g2d.drawString(strs[intV], 140-30, 530-i*3);           
        }
     }      
     gp.moveTo(140,530);
     gp.lineTo(140,20);
