...
class DrawLine95 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,图中给出了梯形的两条边：A2B2C2，请找到第4个顶点，完成这个梯形；";    
           comment2 = "使这个梯形的面积等于给出的平行四边形的面积,你画了" + (clickCount+1) + "次;"; 
        }else{
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交"; 
           comment2 = "";                                                                                  
        }       
     }else{   
        comment = "图中给出了一个平行四边形，和一个梯形的两条边：A2B2C2，请找到第4个顶点完成这个梯形，使其面积等于这个平行四边形的面积。";        
     }
