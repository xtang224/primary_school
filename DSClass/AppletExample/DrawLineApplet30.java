...
public class DrawLineApplet30 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine30());     
   }  
}

class DrawLine30 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){           
           ...
           if (Math.abs(startY-ycenterC)<10 && startX>xcenterC && startX<xcenterB){
              done1 = true;    
              xcenterD = startX;
              ycenterD = ycenterC;          
              gp.moveTo(xcenterB,ycenterB);
        ...
        if (done2){
           ...
           if (aroundPoint(startX2,startY2,xcenterO,ycenterO)){
              done2 = true;
              drawCorrect = true; 
              gp.moveTo(xcenterD,ycenterD);
              gp.lineTo(xcenterO,ycenterO); 
     ...
     if (clickCount>=0){
        if (done1==false){                 
            comment = "你还没有画对,请根据已有的梯形的两条边画出第四个顶点,你画了" + (clickCount+1) + "次;";            
        }else{
           if (done2==false){
              comment = "你已经画出梯形的第四个顶点,请过这个顶点画梯形的高，你画了" + (clickCount+1) + "次;"; 
           }else{
              drawCorrect = true;
        ...
        comment = "请用鼠标点击画线：请根据已有的梯形的两条边画出第四个顶点，完成这个梯形；再过第四个顶点作梯形的高"; 
