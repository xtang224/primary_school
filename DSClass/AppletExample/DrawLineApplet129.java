...
public class DrawLineApplet129 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine129());
     
   }  
}

class DrawLine129 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterO2 = ycenterO - 3*30; // Center position of O2:230
  private static int xcenterA = xcenterO2-85;
  private static int ycenterA = ycenterO2+85; // Center position of A
  private static int xcenterB = xcenterO2-21;
  private static int ycenterB = ycenterO2-56; // Center position of B
  private static int xcenterC = xcenterO2 + 78;
  private static int ycenterC = ycenterO2 - 45; // Center position of C  
  private static int xcenterD = xcenterC+5*30;
  private static int ycenterD = ycenterC; // Center position of D 
  private static int xcenterE = xcenterO+3*30;
  private static int ycenterE = ycenterO-3*30; // Center position of E  
  private static int xcenterF = xcenterO+3*30;
  private static int ycenterF = ycenterO-5*30; // Center position of F  
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("文化广场", xcenterO2+10, ycenterO2-10); 
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){             
           g2d.fillArc(xcenterA-5, ycenterA-5, 10, 10, 0, 360);      
           g2d.drawString("小彬家", xcenterA-10, ycenterA+10);                                         
        }else{
        ...
        if (done2){             
           ...
           g2d.drawString("小丽家", xcenterB-10, ycenterB+10);                                         
        }else{
        ...
        if (done3){             
           g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);      
           g2d.drawString("柳柳家", xcenterC-10, ycenterB+10);                                         
        }else{
           if (aroundPoint(startX3,startY3,xcenterC,ycenterC)){
              done3 = true;
              drawCorrect = true;              
              g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);      
              g2d.drawString("柳柳家", xcenterC-10, ycenterC+10);                                           
           }           
        }                          
     }
     g2d.draw(gp);
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,小彬家在广场西南方向1200米处，请点击选择小彬家所对应的点。你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){            
              comment = "你已经画对了小彬家所对应的点。你画了" + (clickCount+1) + "次;"; 
              comment2 = "小丽家在广场北偏西20度方向600米处，请继续点击选择小丽家所对应的点";
           }else{
              if (done3==false){            
                 comment = "你已经画对了小彬家和小丽家所对应的点。你画了" + (clickCount+1) + "次;"; 
                 comment2 = "柳柳家在广场东偏北30度方向900米处，请继续点击选择柳柳家所对应的点";
              }else{
        ...
        comment = "如图，小彬家在广场西南方向1200米处，小丽家在广场北偏西20度方向600米处，柳柳家在广场东偏北30度方向900米处。";   
        comment2 = "请点击选择小彬家所对应的点。";     
     }
