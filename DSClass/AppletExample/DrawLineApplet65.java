...
public class DrawLineApplet65 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine65());
     
   }  
}

class DrawLine65 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 0;
  private static int ycenterA = 60; // Center position of A
  private static int xcenterB = 330;
  private static int ycenterB = 500; // Center position of B
  private static int xcenterC = 480;
  private static int ycenterC = 530-240; // Center position of C  
  private static int xcenterD = 288;
  private static int ycenterD = 62-7+10; // Center position of D
  private static int xcenterO = 140;
  private static int ycenterO = 530; // Center position of O
  private static int xcenterO2 = xcenterA;
  private static int ycenterO2 = (ycenterA+ycenterO)/2; // Center position of O2
  ...
  public void clearWindow2(Graphics2D g)
  {
    g.setPaint(Color.white);
    g.fill(new Rectangle(0,0,230,getHeight()));
    g.setPaint(Color.black);
  } 
  ...
  public void drawArc(Graphics2D g2d){
     g2d.setPaint(Color.red);
     g2d.drawArc(xcenterA, ycenterA, 480, 460, 0, 360); 
     g2d.drawString("半个篮球场", xcenterA+330, ycenterA-20); 
     ...
     */
     ...
     if (clickCount>=0){
        ...
        if (done1){  
           g2d.fillArc(xcenterB-5, ycenterB-5, 10, 10, 0, 360);
           g2d.drawString("小明",xcenterB-20, ycenterB-20);                       
        }else{
        ...
        if (done2){  
           g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);
           g2d.drawString("小丽",xcenterC-20, ycenterC-20);                       
        }else{
        ...
        if (done3){  
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);
           g2d.drawString("小红",xcenterD-20, ycenterD-20);                       
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到小明的位置，他位于距边线2.5m的三分线上,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了小明的位置,请继续在图中找到小丽的位置，她位于三分线的中点上,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了小明和小丽位置,请继续在图中找到小红的位置，她位于距底线4m的三分线上,你画了" + (clickCount+1) + "次;"; 
              }else
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";                     
           }          
        }       
     }else{   
        comment = "图中给出了篮球场的框架图，请分别找到小明、小丽、小红的位置。";        
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     //gp.moveTo(410,530);
     //gp.lineTo(400,540);   
     g2d.drawString("边线", 500, 530);   
     g2d.drawString("1:250", 230+60, 550);  
     g2d.drawString("三分线", 230, 550);    
     g2d.draw(gp);
  }
