...
public class ChartApplet17 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new ChartLine17());
     
   }  
}

class ChartLine17 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = 320;
  private static int ycenterB = 380; // Center position of B
  private static int xcenterC = 340;
  ...
  public void paint(Graphics g)
  {
    ...
    g2d.drawString("小明家今年1-4月用电情况统计图", 150, 50);  
  }
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        if (done1){  
           g2d.fillRect(xcenterA-200, ycenterA-40, 200, 40);              
        }else{
        ...
        if (done2){
           g2d.fillRect(xcenterB-220, ycenterB-40, 220, 40);            
        }else{
        ...
        if (done3){
           g2d.fillRect(xcenterC-240, ycenterC-40, 240, 40);         
        }else{
        ...
        if (done4){
           g2d.fillRect(xcenterD-280, ycenterD-40, 280, 40);            
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请先画出小明家1月份用电50度的柱状图,你画了" + (clickCount+1) + "次"; 
        }else{
           if (done2==false){
              comment = "你已经画出了小明家1月份用电的柱状图，请继续画2月份用电55度的柱状图。你画了" + (clickCount+1) + "次";
           }else{
              if (done3==false){
                 comment = "你已经画出了小明家1、2月份用电的柱状图，请继续画3月份用电60度的柱状图。你画了" + (clickCount+1) + "次";
              }else{
                 if (done4 == false){
                    comment = "你已经画出了小明家1、2、3月份用电的柱状图，请继续画4月份用电70度的柱状图。你画了" + (clickCount+1) + "次";
                 }else{
                    drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按照提示先后画出小明家今年1-4月份用电情况的柱状图。";
        comment2 = "即画出1月份用电50度、2月份用电55度、3月份用电60度和4月份用电70度的条形统计图。"; 
     }
  ...
  public void drawRuler2(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","1月","","2月","","3月","","4月","",""};
     int intV = 0;
     for (int i=0; i<100; i++){        
        if (i % 10 == 0){
           intV = i/10;
