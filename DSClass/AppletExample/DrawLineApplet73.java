...
public class DrawLineApplet73 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine73());
     
   }  
}

class DrawLine73 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterB = ycenterA+6*30; // Center position of B
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,要按2：1的比例把给出的三角形缩小,已经给出一个顶点，";    
           comment2 = "请按照逆时针方向点击选择和已画出顶点在同一竖直线上的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了2：1缩小三角形的一个顶点";
              comment2 = "请继续按照逆时针方向找2：1缩小三角形的另外一个顶点,你画了" + (clickCount+1) + "次;";
           }else{
        ...
        comment = "图中给出了三角形，现给出2：1缩小的三角形的一个顶点，请按逆时针方向找到其他2个顶点，完成这个三角形。";        
     }
