...
public class DrawLineApplet67 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine67());
     
   }  
}

class DrawLine67 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA = 470; // Center position of A
  private static int xcenterB = xcenterA+3*60;
  private static int ycenterB = ycenterA; // Center position of B
  private static int xcenterC = xcenterB-2*60;
  private static int ycenterC = ycenterB; // Center position of C  
  private static int xcenterD = xcenterA;
  private static int ycenterD = ycenterA-60; // Center position of D
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("街心花园", xcenterA-20, ycenterA-20); 
     /*    
     ...
     */
     ...
     if (clickCount>=0){
        ...
        if (done2){  
           g2d.fillArc(xcenterC-5, ycenterC-5, 10, 10, 0, 360);
           g2d.drawString("图书馆",xcenterC-20, ycenterC-20);                       
        }else{
        ...
        if (done3){  
           g2d.fillArc(xcenterD-5, ycenterD-5, 10, 10, 0, 360);
           g2d.drawString("邮局",xcenterD-20, ycenterD-20);                       
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在图中找到新民小学，位于街心花园正东方向600m,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经找到了新民小学的位置,请继续在图中找到图书馆的位置，位于新民小学正西方向400m,你画了" + (clickCount+1) + "次;"; 
           }else{
              if (done3==false){
                 comment = "你已经找到了新民小学和图书馆位置,请继续在图中找到邮局的位置，位于街心花园正北方向200m,你画了" + (clickCount+1) + "次;"; 
              }else{
        ...
        comment = "图中给出了街心花园的平面图，请分别找到新民小学、图书馆、邮局的位置。";        
     }
  ...
  public void drawGrid(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     for (int i=30; i<=480; i+=60){
        gp.moveTo(i,50);
        gp.lineTo(i,530);
     }
     for (int i=530; i>=50; i-=60){
        gp.moveTo(30,i);
        gp.lineTo(480,i);
     }
     g2d.draw(gp);
