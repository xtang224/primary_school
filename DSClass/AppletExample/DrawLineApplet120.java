...
public class DrawLineApplet120 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine120());
     
   }  
}

class DrawLine120 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB2 = xcenterB+50;
  private static int ycenterB2 = ycenterB+50; // Center position of B
  private static int xcenterC2 = xcenterC+50;
  private static int ycenterC2 = ycenterC+50; // Center position of C   
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("家接下水管处", xcenterA-20, ycenterA-20);
     g2d.drawString("污水排水渠", (xcenterB+xcenterC)/2, (ycenterB+ycenterC)/2+20);
     ...
     if (clickCount>=0){
        if (drawCorrect==false){
           if (perpendicular(startX, startY, xcenterA, ycenterA, xcenterB, ycenterB, xcenterC, ycenterC)){  
              ...
              comment = "你还没有画对,请继续点击画线：画出过家接下水管处到污水排水渠的垂线,你画了" + (clickCount+1)/2 + "次;"; 
           } 
        }else
           comment = "你完全画对了,你画了" + (clickCount+1)/2 + "次;请返回原答题页面点击按钮完成填空，然后提交";
     }else{  
        comment = "你还没有开始画，请用鼠标点击画线：画出过家接下水管处到污水排水渠的垂线"; 
     }
