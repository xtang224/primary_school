...
public class DrawLineApplet18 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine18());
     
   }  
}

class DrawLine18 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     if (clickCount>=0){
        ...
        if (done2){
           ...
           if (clickCount>=1 && !aroundPoint(startX2, startY2, xcenterA, ycenterA) && startX2<xcenterA && startY2>ycenterA){
              done2 = true;                   
              xcenterC = xcenterB - (xcenterA - startX2);
              ycenterC = startY2;    
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(startX2,startY2);     
              gp.moveTo(xcenterC,ycenterC);
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请画通过顶点A的平行四边形的横边,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画对了通过顶点A的平行四边形的横边,请继续画通过顶点A的平行四边形的另外一条边，你画了" + (clickCount+1) + "次;";
           }else{ 
              if (done3==false){
                 comment = "你已经画对了平行四边形的横边,请继续画通过顶点A的垂直于横边的平行四边形的高，你画了" + (clickCount+1) + "次;";
              }else{                                
                 drawCorrect = true;
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面并在答案中输入：" + target + "，然后提交";
              }              
           }        
        }              
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示画通过顶点A的平行四边形的横边；";
        comment2 = "再画过顶点A的平行四边形的竖边，最后画一条过顶点A的平行四边形的高。"; 
     }
