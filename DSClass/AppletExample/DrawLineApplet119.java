...
public class DrawLineApplet119 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine119());
     
   }  
}

class DrawLine119 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = xcenterA+240;
  private static int ycenterB = ycenterA; // Center position of B
  private static int xcenterC = xcenterB+60;
  private static int ycenterC = ycenterB+180; // Center position of C  
  private static int xcenterD = xcenterC-240;
  private static int ycenterD = ycenterC; // Center position of D
  private static int xcenterE = xcenterB;
  private static int ycenterE = ycenterC; // Center position of E 
     ...
     */
     if (clickCount>=0){
        ...
        if (done1){  
           ...
           g2d.drawString("E", xcenterE+20, ycenterE+20);                   
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在平行四边形中从点B向DC作高,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";          
        }else{
           if (done2==false){
              comment = "你已经画出了从点B向DC作的高BE，请以BE为一条直角边,";
              comment2 = "在平行四边形内画出一个面积最大的直角三角形，你画了" + (clickCount+1) + "次;";              
           }else{
        ...
        comment = "图中给出了平行四边形，请从点B向DC作高，并以这条高为一条直角边";
        comment2 = "在平行四边形内画出一个面积最大的直角三角形，并涂上阴影;";             
     }
