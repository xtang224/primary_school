...
public class DrawLineApplet63 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine63());
     
   }  
}

class DrawLine63 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 180;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){  
           ...
           if (aroundPoint(startX,startY,xcenterA2,ycenterA2) || aroundPoint(startX,startY,xcenterB2,ycenterB2) || aroundPoint(startX,startY,xcenterC2,ycenterC2) || aroundPoint(startX,startY,xcenterD2,ycenterD2)){
              done1 = true; 
              drawCorrect = true; 
              gp.moveTo(xcenterA2,ycenterA2);
              gp.lineTo(xcenterB2,ycenterB2); 
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在点A的右侧找到另外一个正方形的顶点,你画了" + (clickCount+1) + "次;"; 
        }else{           
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮直接提交";                     
        }       
     }else{   
        comment = "图中给出了看物体的一个长方形，请在点A的右侧找到相应的顶点，画出其余的图形。";        
     }
