...
public class DrawLineApplet36 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine36());
     
   }  
}

class DrawLine36 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterO2 =100;
  private static int ycenterO2 = 500;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){           
           ...
           if (parallel(startX, startY, xcenterO,ycenterO, xcenterA,ycenterA,xcenterB,ycenterB)){
              done1 = true;
              gp.moveTo(xcenterO,ycenterO);
              gp.lineTo(xcenterO2,ycenterO2);
           }           
     ...
     if (clickCount>=0){
        if (done1==false){           
           comment = "你还没有画对,请继续点击过点O画直线AB的平行线,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示依次过点O画直线AB的平行线和直线AC的垂线"; 
     }
