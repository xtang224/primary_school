...
public final class SmartRandomAction extends Action {
    ...
    throws Exception {
        ...
        String classId = (String)((SmartRandomForm) form).getClassId();
               ...
               if (!userName.equals("2000")){
                  ...
                  if (status == null || status.equals("") || status.equals("not active")){
                    ...
                    //now we want to initiate StringSetTransfer.hs_middle_fb_used,hs_middle_fb,total_fb                    
                     ...
                     stmt = myConnection.prepareStatement("select problemId from usedMultiply_test where source=?");
                     stmt.clearParameters();                     
                     ...
                     stmt = myConnection.prepareStatement("select problemId from usedDivision_test where source=?");
                     stmt.clearParameters();                     
                     stmt.setString(1, source);
                     rs = stmt.executeQuery();                     
                     while (rs.next()){
                     ...
                     stmt = myConnection.prepareStatement("select problemId from usedPlusMinus where source=?");
                     stmt.clearParameters();                     
                     stmt.setString(1, source);
                     rs = stmt.executeQuery();                     
                     while (rs.next()){
...
System.out.println("the total number of questions in this test: " + (StringSetTransfer.hs_middle_fb.size()+StringSetTransfer.hs_high_fdb.size()+StringSetTransfer.hs_high_ftb.size()+StringSetTransfer.hs_high_fqb.size()+StringSetTransfer.mhs_high.size()+StringSetTransfer.hs_low_tf.size()+StringSetTransfer.hs_low.size()+StringSetTransfer.hs_high_m.size()+StringSetTransfer.hs_high_d.size()+StringSetTransfer.hs_middle_pm.size()) + "");
            ...
            String answeredHashSetStr_ftb = (String)((SmartRandomForm) form).getAnsweredHashSet_ftb();
            ...
            String answeredHashSetStr_m = (String)((SmartRandomForm) form).getAnsweredHashSet_m();  
            hs_m = StringSetTransfer.stringToSet(answeredHashSetStr_m);
            String answeredHashSetStr_d = (String)((SmartRandomForm) form).getAnsweredHashSet_d();  
            hs_d = StringSetTransfer.stringToSet(answeredHashSetStr_d);
            String answeredHashSetStr_pm = (String)((SmartRandomForm) form).getAnsweredHashSet_pm();  
     ...
     if (id != 0){
        ...
        System.out.println("SmartRandomAction, hs_ftb str is " + StringSetTransfer.setToString(hs_ftb));  
        ...
        System.out.println("SmartRandomAction, hs_pm str is " + StringSetTransfer.setToString(hs_pm));  
