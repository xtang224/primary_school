...
public final class SmartRandom5Action extends Action {
    ...
    throws Exception {
        ...
        PersonBean pb2 = null;
        pb2 = new PersonBean();
        String userName = (String)((SmartRandom5Form) form).getUserName();
        String passWord = (String)((SmartRandom5Form) form).getPassWord();
        String trueName = (String)((SmartRandom5Form) form).getTrueName();
        String playerId = (String)((SmartRandom5Form) form).getPlayerId();
        pb2.setUserName(userName);
        pb2.setPassWord(passWord); 
        pb2.setTrueName(trueName);
        pb2.setPlayerId(playerId);  

        String times = (String)((SmartRandom5Form) form).getTimes();
        pb2.setTimes(times);

        String planStatus = (String)((SmartRandom5Form) form).getPlanStatus();
        pb2.setPlanStatus(planStatus);

        String classId = (String)((SmartRandom5Form) form).getClassId();
        pb2.setClassId(classId);

        String source = (String)((SmartRandom5Form) form).getSource();
               ...
               if (!userName.equals("2000")){
                  ...
                  if (status == null || status.equals("") || status.equals("not active")){
                    ...
                    //now we want to initiate StringSetTransfer.hs_middle_fb_used,hs_middle_fb,total_fb                   
                     ...
                     //now we want to initiate StringSetTransfer.hs_high_ftb_used,hs_high_ftb,total_ftb                     
                     StringSetTransfer.hs_high_ftb_used = hs_high_ftb_used;
                     rs = s.executeQuery("select id from fillTripleBlanks2_test where type='H' and source='" + source + "'");
                     while (rs.next()){
                        int usedId = rs.getInt("id");
                        hs_high_ftb.add(new Integer(usedId));
                     }
                     hs_high_ftb.removeAll(hs_high_ftb_used);
                     StringSetTransfer.hs_high_ftb = hs_high_ftb;
                     rs = s.executeQuery("select id from fillTripleBlanks2_test where source='"+ source + "'");         
                     numberId = 0;
                     while (rs.next()){
                        int usedId = rs.getInt("id");
                        numberId ++;                 
                     ...
                     StringSetTransfer.total_ftb = numberId;
                     ...
                     rs = s.executeQuery("select id from multiply_test where source='"+ source + "'");         
                     numberId = 0;
                     while (rs.next()){
                        int usedId = rs.getInt("id");
                        numberId ++;                      
                     }  
                     StringSetTransfer.total_m = numberId;
                     ...
                     rs = s.executeQuery("select id from division_test where source='"+ source + "'");         
                     numberId = 0;
                     while (rs.next()){
                        int usedId = rs.getInt("id");
                        numberId ++;                      
                     }  
                     StringSetTransfer.total_d = numberId;

                     //now we want to initiate StringSetTransfer.hs_middle_pm, hs_middle_pm_used, total_pm
                     
                     StringSetTransfer.hs_middle_pm_used = hs_middle_pm_used;
                     rs = s.executeQuery("select id from plusMinus where source='" + source + "'");
                     while (rs.next()){
                        int usedId = rs.getInt("id");
                        hs_middle_pm.add(new Integer(usedId));
                     }
                     ...
                     StringSetTransfer.total_pm = numberId;
                  
System.out.println("the total number of raw questions: " + (StringSetTransfer.total_fb+StringSetTransfer.total_fdb+StringSetTransfer.total_ftb+StringSetTransfer.total_fqb+StringSetTransfer.mtotal+StringSetTransfer.total_tf+StringSetTransfer.total+StringSetTransfer.total_m+StringSetTransfer.total_d+StringSetTransfer.total_pm) + "");
System.out.println("the total number of questions in this test: " + (StringSetTransfer.hs_middle_fb.size()+StringSetTransfer.hs_high_fdb.size()+StringSetTransfer.hs_high_ftb.size()+StringSetTransfer.hs_high_fqb.size()+StringSetTransfer.mhs_high.size()+StringSetTransfer.hs_low_tf.size()+StringSetTransfer.hs_low.size()+StringSetTransfer.hs_high_m.size()+StringSetTransfer.hs_high_d.size()+StringSetTransfer.hs_middle_pm.size()) + "");
            ...
            String answeredHashSetStr = (String)((SmartRandom5Form) form).getAnsweredHashSet();
            hs = StringSetTransfer.stringToSet(answeredHashSetStr);
            String answeredHashSetStr_tf = (String)((SmartRandom5Form) form).getAnsweredHashSet_tf();
            hs_tf = StringSetTransfer.stringToSet(answeredHashSetStr_tf);
            String answered_M_HashSetStr = (String)((SmartRandom5Form) form).getAnswered_M_HashSet();
            mhs = StringSetTransfer.stringToSet(answered_M_HashSetStr);
            String answeredHashSetStr_fb = (String)((SmartRandom5Form) form).getAnsweredHashSet_fb();
            hs_fb = StringSetTransfer.stringToSet(answeredHashSetStr_fb);            
            String answeredHashSetStr_fdb = (String)((SmartRandom5Form) form).getAnsweredHashSet_fdb();
            hs_fdb = StringSetTransfer.stringToSet(answeredHashSetStr_fdb);
            String answeredHashSetStr_ftb = (String)((SmartRandom5Form) form).getAnsweredHashSet_ftb();
            hs_ftb = StringSetTransfer.stringToSet(answeredHashSetStr_ftb);
            String answeredHashSetStr_fqb = (String)((SmartRandom5Form) form).getAnsweredHashSet_fqb();
            hs_fqb = StringSetTransfer.stringToSet(answeredHashSetStr_fqb);
            String answeredHashSetStr_tm = (String)((SmartRandom5Form) form).getAnsweredHashSet_tm();  
            hs_tm = StringSetTransfer.stringToSet(answeredHashSetStr_tm);           
            String answeredHashSetStr_m = (String)((SmartRandom5Form) form).getAnsweredHashSet_m();  
            hs_m = StringSetTransfer.stringToSet(answeredHashSetStr_m);
            String answeredHashSetStr_d = (String)((SmartRandom5Form) form).getAnsweredHashSet_d();  
            hs_d = StringSetTransfer.stringToSet(answeredHashSetStr_d);
            String answeredHashSetStr_pm = (String)((SmartRandom5Form) form).getAnsweredHashSet_pm();  
        ...
        String lastType = (String)((SmartRandom5Form) form).getLastType();
        String thisType = (String)((SmartRandom5Form) form).getThisType();
        String continueRight = (String)((SmartRandom5Form) form).getContinueRight();
        String continueWrong = (String)((SmartRandom5Form) form).getContinueWrong();
        int intContinueRight = Integer.parseInt(continueRight);
        int intContinueWrong = Integer.parseInt(continueWrong);
        String neverHigh = (String)((SmartRandom5Form) form).getNeverHigh();
        boolean boolNeverHigh = Boolean.parseBoolean(neverHigh);
        System.out.println("Inside SmartRandom5Action.java, lastType = " + lastType);
        ...
        String lastCorrect = (String)((SmartRandom5Form) form).getLastCorrect();
        System.out.println("Inside SmartRandom5Action.java, lastCorrect = " + lastCorrect);
        ...
        while (true){           
             ...
             //up to this point, we have decided the value of thisChar for both first==true and first!=true
                 ...
                 System.out.println("Inside SmartRandom5Action, thisType = " + thisType);
                 //below we will not care about any used set when we decide whether low,middle,high are used up
                 switch (thisType.charAt(0)){
                    case 'L':
                       System.out.println("Inside SmartRandom5Action, case 'L', thisType = " + thisType);
                       ...
                       break;
                    case 'M':
                       System.out.println("Inside SmartRandom5Action, case 'M', thisType = " + thisType);
                       ...
                       if (hs_fb.containsAll(StringSetTransfer.hs_middle_fb)){
                          if (hs_tm.containsAll(StringSetTransfer.hs_middle_tm)){
                             if (hs_pm.containsAll(StringSetTransfer.hs_middle_pm)){
                                System.out.println("Inside SmartRandom5Action, case 'M', this means all middle problems are ex = " + thisType);
                       ...
                       break;
                    case 'H':
                       System.out.println("Inside SmartRandom5Action, case 'H', thisType = " + thisType);
     ...
     System.out.println("Inside SmartRandom5Action, after id and randomNumber is obtained, id = " + id + " and randomNumber = " + randomNumber);
     ...
     if (id != 0){
           ...
           switch (randomNumber){
              case 0:
               ...
               case 1:
                  ...
                  //getServlet().log("inside SmartRandom5Action.java, when select * from choices2 where id=?,  id = " + id);
                  stmt.clearParameters();
                  stmt.setInt(1, id);
                  rs = stmt.executeQuery();
                  while (rs.next()){
               ...
               case 7:
                  ...
                  System.out.println("at initiation in SmartRandomActioin: ftbb is reached");
                  break; 
               case 8:
                  ...
                  System.out.println("at initiation in SmartRandomActioin: mb is reached");       
                  break; 
               case 9:
                  ...
                  System.out.println("at initiation in SmartRandomActioin: db is reached");
                  break;  
               case 10:
                  ...
                  System.out.println("at initiation in SmartRandomActioin: pmb is reached");       
                  break;            
               default:
                  break;
        ...
        System.out.println("at SmartRandom5Action, hs string is " + StringSetTransfer.setToString(hs));
        System.out.println("SmartRandom5Action, hs_tf str is " + StringSetTransfer.setToString(hs_tf));
        System.out.println("SmartRandom5Action, mhs str is " + StringSetTransfer.setToString(mhs));   
        System.out.println("SmartRandom5Action, hs_fb str is " + StringSetTransfer.setToString(hs_fb));   
        System.out.println("SmartRandom5Action, hs_fdb str is " + StringSetTransfer.setToString(hs_fdb));
        System.out.println("SmartRandom5Action, hs_ftb str is " + StringSetTransfer.setToString(hs_ftb));  
        System.out.println("SmartRandom5Action, hs_fqb str is " + StringSetTransfer.setToString(hs_fqb));
        System.out.println("SmartRandom5Action, hs_tm str is " + StringSetTransfer.setToString(hs_tm));                      
        System.out.println("SmartRandom5Action, hs_m str is " + StringSetTransfer.setToString(hs_m));                      
        System.out.println("SmartRandom5Action, hs_d str is " + StringSetTransfer.setToString(hs_d));
        System.out.println("SmartRandom5Action, hs_pm str is " + StringSetTransfer.setToString(hs_pm));  
        
        String answeredProblems = (String)((SmartRandom5Form) form).getAnsweredProblems();
        String correctAnswers = (String)((SmartRandom5Form) form).getCorrectAnswers();
        ...
        String totalScore = (String)((SmartRandom5Form) form).getTotalScore();
        String correctAnswers_low = (String)((SmartRandom5Form) form).getCorrectAnswers_low();
        String answeredProblems_low = (String)((SmartRandom5Form) form).getAnsweredProblems_low();
        ...
        String correctAnswers_middle = (String)((SmartRandom5Form) form).getCorrectAnswers_middle();
        String answeredProblems_middle = (String)((SmartRandom5Form) form).getAnsweredProblems_middle();        
        ...
        String correctAnswers_high = (String)((SmartRandom5Form) form).getCorrectAnswers_high();
        String answeredProblems_high = (String)((SmartRandom5Form) form).getAnsweredProblems_high();        
        ...
        while(true){
           try{
              ...
              getServlet().log("Inside SmartRandom5Action.java, process.", ex);
           }finally{
              try{
                 if (rs != null) rs.close();
                 if (s != null) s.close();
                 ...
                 getServlet().log("Inside SmartRandom5Action.java, closing.", e);
              }
           } 
        }
        ...
        while (iter.hasNext()) {
           if (index == selectNumber){
              ...
              System.out.println("inside SmartRandom5Action, the selected student = " + tempUserName);
              break;
           }
           iter.next();
           index++;                      
