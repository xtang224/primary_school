...
public class DrawLineApplet28 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine28());     
   }  
}

class DrawLine28 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = xcenterA-100;
  private static int ycenterB = ycenterA+300; // Center position of B
  private static int xcenterC = xcenterB+500;
  private static int ycenterC = ycenterB; // Center position of C  
  private static int xcenterD = xcenterA+500;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done2){
           ...
           if (inLine(xcenterB,ycenterB,xcenterC,ycenterC,startX2,startY2) && startX2>xcenterB && startX2<xcenterC && !aroundPoint(startX2,startY2,xcenterH,ycenterH)){
              done2 = true;
              drawCorrect = true;  
              xcenterO2 = startX2;
              ycenterO2 = ycenterB;
     ...
     if (clickCount>=0){
        if (done1==false){            
           ...
           if (done2==false){
              comment = "你已经在AD上找到了一点,请继续在BC上找另外一点，组成线段，把图形分成两个梯形，你画了" + (clickCount+1) + "次;"; 
           }else{
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请在AD和BC上各找一点，画一条线段，把图形分成两个梯形。";        
     }
