...
public class DrawLineApplet5 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine5());     
   }  
}

class DrawLine5 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=1){        
        if (inLine(startX, startY, endX, endY, xcenterA, ycenterA)){
           if (target.equalsIgnoreCase("180")){
              comment = "你画对了,你画了" + clickCount + "次;请返回原答题页面点击按钮完成填空，然后提交"; 
              drawCorrect = true;
              //below we are goind to draw an arc of 180°
              double tValue = -((double)(endY-startY)) / (endX-startX);
              double tangent = Math.atan(tValue);
              System.out.println("tValue = " + tValue + " and tangent = " + tangent);
              int startAngle = 0;
              if (tangent>0)
                 startAngle = (int)(tangent / Math.PI * 180);
              else
                 startAngle = (int)(tangent / Math.PI * 180) + 360;
              System.out.println("startAngle = " + startAngle);
              g2d.setStroke(new BasicStroke(1.0f));           
              g2d.drawArc(xcenterA-10, ycenterA-10, 20, 20, startAngle, 180);
           }else if (target.equalsIgnoreCase("360")){
              if ((Math.abs(startX-xcenterA)<=5 && Math.abs(startY-ycenterA)<=5) || (Math.abs(endX-xcenterA)<=5 && Math.abs(endY-ycenterA)<=5)){
                 comment = "你画对了,你画了" + clickCount + "次;请返回原答题页面点击按钮完成填空，然后提交"; 
                 drawCorrect = true;
                 g2d.setStroke(new BasicStroke(1.0f)); 
        ...
        if (target.equalsIgnoreCase("180"))
           comment = "你还没有开始画，请先画一条过点A的直线，然后画弧"; 
        else if (target.equalsIgnoreCase("360"))
           comment = "你还没有开始画，请先画一条从A出发的射线，然后画弧";   
     }
