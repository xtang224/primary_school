...
public class DrawLineApplet26 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine26());     
   }  
}

class DrawLine26 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterE = 500;
  private static int ycenterE = 100; // Center position of E  
  private static int xcenterC2 = 50;
  private static int ycenterC2 = 400; // Center position of C
  private static int xcenterD2 = 350;
  private static int ycenterD2 = 400; // Center position of D  
  private static int xcenterE2 = xcenterE;
  private static int ycenterE2 = ycenterE+100; // Center position of E2 
  private static int xcenterO = 100;
  private static int ycenterO = 300; // Center position of O
  private static int xcenterO2 = 356;
  private static int ycenterO2 = 392; // Center position of O2
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("B村", xcenterB+20, ycenterB+20);
     g2d.drawString("公路", xcenterD, (ycenterD+ycenterD2)/2);        
     ...
     if (clickCount>=0){
        if (done1==false){           
           comment = "你还没有画对,请继续点击画A村到公路的垂线,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画了A村到公路的垂线,请继续画B村到公路的垂线，你画了" + (clickCount+1) + "次;"; 
           }else{                        
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示画A村到公路的垂线，再画B村到公路的垂线"; 
     }
