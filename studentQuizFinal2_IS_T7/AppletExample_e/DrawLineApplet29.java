...
public class DrawLineApplet29 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine29());     
   }  
}

class DrawLine29 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = xcenterA-300;
  private static int ycenterB = ycenterA+300; // Center position of B
  private static int xcenterC = xcenterB+600;
  ...
  private static int xcenterO = (xcenterB+xcenterD)/2;
  private static int ycenterO = ycenterA; // Center position of O
  private static int xcenterO2 = (xcenterB+xcenterD)/2;
  private static int ycenterO2 = ycenterB; // Center position of O2
  private static int xcenterH = 0;
  private static int ycenterH = 0;
  private static int xcenterH2 = 0;
  private static int ycenterH2 = 0;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){  
           ...
           if (inLine(xcenterA,ycenterA,xcenterD,ycenterD,startX,startY) && startX>xcenterA && startX<xcenterD){
              done1 = true;  
              xcenterO = startX;
              ycenterO = ycenterA;
              xcenterH = xcenterB + (xcenterO-xcenterA);    
              ycenterH = ycenterB; 
              xcenterH2 = xcenterC - (xcenterD-xcenterO);    
              ycenterH2 = ycenterB;     
              g2d.fillArc(xcenterO-5, ycenterO-5, 10, 10, 0, 360);              
           }           
        }
        ...
        if (done2){
           ...
           if (aroundPoint(startX2,startY2,xcenterH,ycenterH)){
              done2 = true;
              drawCorrect = true;  
              xcenterO2 = xcenterH;
              ycenterO2 = ycenterH;
              gp.moveTo(xcenterO,ycenterO);
              gp.lineTo(xcenterO2,ycenterO2); 
           }else if (aroundPoint(startX2,startY2,xcenterH2,ycenterH2)){
              done2 = true;
              drawCorrect = true;  
              xcenterO2 = xcenterH2;
              ycenterO2 = ycenterH2;
     ...
     if (clickCount>=0){
        if (done1==false){            
           comment = "你还没有画对,请在AD上找一点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经在AD上找到了一点,请继续在BC上找另外一点，组成线段，把图形分成一个平行四边形和一个梯形，你画了" + (clickCount+1) + "次;"; 
           }else{
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请在AD和BC上各找一点，画一条线段，把图形分成一个平行四边形和一个梯形。";        
     }
