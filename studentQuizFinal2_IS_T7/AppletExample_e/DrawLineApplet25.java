...
public class DrawLineApplet25 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine25());     
   }  
}

class DrawLine25 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请点击点O画线段，把图形分成两个完全相同的梯形,你画了" + (clickCount+1) + "次;"; 
        }else{
           drawCorrect = true;
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交";                    
        }       
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请点击点O画一条线段，把图形分成两个完全相同的梯形。";        
     }
