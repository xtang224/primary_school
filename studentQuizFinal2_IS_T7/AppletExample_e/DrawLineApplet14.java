...
public class DrawLineApplet14 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine14());     
   }  
}

class DrawLine14 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = 100+400+40;
  private static int ycenterB = 500-40; // Center position of B
  private static int xcenterC = 100+40;
  private static int ycenterC = 500-40-300; // Center position of C  
  private static int xcenterD = 100+400+40;
  private static int ycenterD = 500-40-300; // Center position of D
  private static int xcenterO = 300+40;
  private static int ycenterO = 350-40; // Center position of O
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("B", xcenterB+20, ycenterA+20);
     ...
     if (clickCount>=0){
        ...
        if (done3){
           ...
           if (aroundPoint(startX3, startY3, xcenterO,ycenterO)){
              ...
              drawCorrect = true;
              g2d.fillArc(xcenterO-5, ycenterO-5, 10, 10, 0, 360);      
              g2d.drawString("O", xcenterO-10, ycenterO+20);               
           }
        }
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请点击点A,画从A出发的长方形的对角线,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你只画对了一条对角线,请继续点击点B,画从B出发的长方形的对角线,你画了" + (clickCount+1) + "次;";
           }else{ 
              if (done3==false){
                 comment = "你画对了两条对角线,请继续点击这两条对角线的交点,该点就是这个长方形的正中央,你画了" + (clickCount+1) + "次;";
              }else{                       
                 drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示先点击点A，画从A出发的长方形的对角线；";
        comment2 = "再点击点B,画从B出发的长方形的对角线；再点击两个对角线的交点，即为这个长方形的正中央"; 
     }
