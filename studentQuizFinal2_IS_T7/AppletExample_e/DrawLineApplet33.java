...
public class DrawLineApplet33 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine33());     
   }  
}

class DrawLine33 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterC = xcenterA+80+40;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请根据已有的等腰梯形的一条左腰先画出和A同下底（5厘米）的另外一个顶点,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画出了另一个顶点，请继续画出上底（1厘米）的第四个顶点,完成这个梯形。你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请根据已有的等腰梯形的左腰先画出和A同下底（5厘米）的第三个顶点，";
        comment2 = "再画和B同上底（1厘米）的第四个顶点，完成这个等腰梯形；最后画从B出发的等腰梯形的高。"; 
     }
