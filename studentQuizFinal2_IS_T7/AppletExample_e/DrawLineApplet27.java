...
public class DrawLineApplet27 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine27());     
   }  
}

class DrawLine27 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = 500;
  private static int ycenterB = 100; // Center position of B
  private static int xcenterC = 700;
  private static int ycenterC = 500; // Center position of C
  private static int xcenterD = 200;
  private static int ycenterD = 100;
  private static int xcenterE = 400;
  private static int ycenterE = 500;
  private static int xcenterO = 540;
  private static int ycenterO = 180;
  ...
  DrawLine27()
  {
    ...
    if (target.equals("垂直平行A")){
       xcenterA = 300; ycenterA = 300;
       xcenterB = 500; ycenterB = 100;
       xcenterC = 700; ycenterC = 500;
       xcenterD = 200; ycenterD = 100;
       xcenterE = 400; ycenterE = 500;
       xcenterO = 540; ycenterO = 180;
    }else if (target.equals("垂直平行B")){
       xcenterA = 540; ycenterA = 180;
       xcenterB = 200; ycenterB = 100;
       xcenterC = 400; ycenterC = 500;
       xcenterD = 500; ycenterD = 100;
       xcenterE = 700; ycenterE = 500;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     g2d.drawString("P", xcenterA-20, ycenterA-20);
     g2d.drawString("l", xcenterB-20, ycenterB-20);
     ...
     if (clickCount>=0){
        if (done1==false){
           if (perpendicular(startX, startY, xcenterA, ycenterA, xcenterB, ycenterB, xcenterC, ycenterC)  && inLine(xcenterB,ycenterB,xcenterC,ycenterC,startX,startY)){   
              done1 = true;             
              comment = "你画对了点P到直线l的垂线m,请继续画m的垂线，你画了" + (clickCount+1) + "次;";              
           }else          
              comment = "你还没有画对,请继续画点P到直线l的垂线,你画了" + (clickCount+1) + "次;"; 
        }else{  
           if (done2==false){
              if (perpendicular(startX2, startY2, xcenterA, ycenterA, xcenterA, ycenterA, xcenterO, ycenterO)  && inLine(xcenterD,ycenterD,xcenterE,ycenterE,startX2,startY2)){
                 done2 = true;
                 drawCorrect = true;
                 comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交";
              }else
                 comment = "你还没有完全画对，你画对了点P到直线l的垂线m,请继续画m的垂线，你画了" + (clickCount+1) + "次;"; 
           }else{  
              drawCorrect = true;      
              comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交";   
           }         
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请先画出过点A垂直于直线l的垂线m，再画出过点A垂直于m的垂线"; 
     }
     ...
     if (clickCount>=0){
        ...
        if (done1){
           ...
           g2d.drawString("m", (xcenterA+xcenterO)/2-20, (ycenterA+ycenterO)/2-20);          
        }  
