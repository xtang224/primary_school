...
public final class SmartQuitAction extends Action {
    ...
    throws Exception {
        ...
        String source = (String)((SmartQuitForm) form).getSource();
        ...
        String answeredHashSetStr_my = (String)((SmartQuitForm) form).getAnswered_MY_HashSet();
        HashSet mb = StringSetTransfer.stringToSet(answeredHashSetStr_my);
        mb.remove(new Integer(0));

        String answeredHashSetStr_tm = (String)((SmartQuitForm) form).getAnsweredHashSet_tm();
        HashSet tmb = StringSetTransfer.stringToSet(answeredHashSetStr_tm);
        tmb.remove(new Integer(0));

        String answeredHashSetStr_d = (String)((SmartQuitForm) form).getAnswered_D_HashSet();
        HashSet md = StringSetTransfer.stringToSet(answeredHashSetStr_d);
        md.remove(new Integer(0));

        String answeredHashSetStr_c = (String)((SmartQuitForm) form).getAnswered_C_HashSet();
        HashSet mc = StringSetTransfer.stringToSet(answeredHashSetStr_c);
        mc.remove(new Integer(0));
        ...
        getServlet().log("SmartQuitAction, mb string is " + answeredHashSetStr_my);
        getServlet().log("SmartQuitAction, tmb string is " + answeredHashSetStr_tm);
        getServlet().log("SmartQuitAction, md string is " + answeredHashSetStr_d);
        getServlet().log("SmartQuitAction, mc string is " + answeredHashSetStr_c);

        //now we want to get FullTotalScore of the test
        double dblFullScore = 0;
        Object[] obs = hs.toArray();
        Integer tmpInteger = new Integer(0);
        Integer tmpInteger2 = new Integer(0);
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_q.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = hs_tf.toArray();
        tmpInteger = new Integer(0);
        tmpInteger2 = new Integer(0);
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_c.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = mhs.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_mq.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        } 
        obs = hs_fb.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_fb.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = hs_fdb.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_fdb.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }  
        obs = mb.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_m.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = tmb.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_tm.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = md.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_d.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        }
        obs = mc.toArray();
        tmpInteger = new Integer(0);
        ...
        for (int i=0; i<obs.length; i++){
           tmpInteger = (Integer)obs[i];
           tmpInteger2 = (Integer)StringSetTransfer.hm_c2.get(tmpInteger);
           dblFullScore += tmpInteger2.intValue();
        } 
        dbTotalScore = dbTotalScore / dblFullScore * 100;
        pb2.setTotalFullScore(dblFullScore);
        pb2.setTotalScore(dbTotalScore);
        
        System.out.println("Inside SmartQuitAction, dblFullScore = " + dblFullScore);       
...
stmt = myConnection.prepareStatement("update student_score set totalscore=?, answeredProblems=?, correctAnswers=?, answeredProblems_low=?, correctAnswers_low=?, answeredProblems_middle=?, correctAnswers_middle=?, answeredProblems_high=?, correctAnswers_high=?, planStatus=? where studentNo=?");  
           ...
           //we also want to update hintMatch about totalScore2
           stmt = myConnection.prepareStatement("update hintMatch set term2=? where term1=?");
           stmt.clearParameters();
           stmt.setString(1, "" + dbTotalScore);
           stmt.setString(2, "totalScore2");
           stmt.executeUpdate();              
