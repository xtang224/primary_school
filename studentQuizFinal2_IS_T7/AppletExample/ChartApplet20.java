...
public class ChartApplet20 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new ChartLine20());
     
   }  
}

class ChartLine20 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA = 500-100; // Center position of A
  private static int xcenterB = 220;
  private static int ycenterB = 500-160; // Center position of B
  private static int xcenterC = 300;
  private static int ycenterC = 500-11*20; // Center position of C  
  private static int xcenterD = 380;
  private static int ycenterD = 500-100; // Center position of D
  private static int xcenterE = 460;
  private static int ycenterE = 500-40; // Center position of E
  ...
  public void paint(Graphics g)
  {
    ...
    g2d.drawString("某城市今年10月空气质量统计图", 200, 100);  
  }
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        if (done1){  
           g2d.fillRect(xcenterA, ycenterA, 40, 100);              
        }else{
        ...
        if (done3){
           g2d.fillRect(xcenterC, ycenterC, 40, 11*20);         
        }else{
        ...
        if (done4){
           g2d.fillRect(xcenterD, ycenterD, 40, 100);            
        }else{
        ...
        if (done5){
           g2d.fillRect(xcenterE, ycenterE, 40, 40);            
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请先画出5天空气优的柱状图,你画了" + (clickCount+1) + "次"; 
        }else{
           if (done2==false){
              comment = "你已经画出了空气优的柱状图，请继续画8天良的柱状图。你画了" + (clickCount+1) + "次";
           }else{
              if (done3==false){
                 comment = "你已经画出了空气优、良的柱状图，请继续画11天轻度污染的柱状图。你画了" + (clickCount+1) + "次";
              }else{
                 if (done4==false){
                    comment = "你已经画出了空气优、良、轻度污染的柱状图，请继续画5天中度污染的柱状图。你画了" + (clickCount+1) + "次";
                 }else{
                    if (done5==false){
                       comment = "你已经画出了空气优、良、轻度污染、中度污染的柱状图，请继续画2天高度污染的柱状图。你画了" + (clickCount+1) + "次";
                    }else{
                       drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按照提示画出某城市今年10月的空气质量的柱状图。";
        comment2 = "即画出5天优、8天良、11天轻度污染、5天中度污染和2天重度污染的条形统计图。"; 
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","优","","良","","轻度","","中度","","重度",""};
     String[] strs2 = new String[]{"","","","","","污染","","污染","","污染",""};
     ...
     gp.moveTo(540,500);
     gp.lineTo(530,510);   
     g2d.drawString("空气质量", 540, 520);
     g2d.drawString("等级", 540, 540);
     g2d.draw(gp);
  }
