...
public class ChartApplet2 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new ChartLine2());
     
   }  
}

class ChartLine2 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterA = 140; // Center position of A
  private static int xcenterB = 190;
  private static int ycenterB = 140; // Center position of B
  private static int xcenterC = 250;
  private static int ycenterC = 380; // Center position of C  
  private static int xcenterD = 310;
  private static int ycenterD = 420; // Center position of D
  private static int xcenterE = 370;
  private static int ycenterE = 380; // Center position of E 
  private static int xcenterF = 430;
  private static int ycenterF = 460; // Center position of F 
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        if (done1){  
           g2d.fillRect(xcenterA, ycenterA, 30, 360);              
        }else{
        ...
        if (done2){
           g2d.fillRect(xcenterB, ycenterB, 30, 360);            
        }else{
        ...
        if (done3){
           g2d.fillRect(xcenterC, ycenterC, 30, 120);         
        }else{
        ...
        if (done4){
           g2d.fillRect(xcenterD, ycenterD, 30, 80);         
        }else{
        ...
        if (done5){
           g2d.fillRect(xcenterE, ycenterE, 30, 120);         
        }else{
        ...
        if (done6){
           g2d.fillRect(xcenterF, ycenterF, 30, 40);         
        }else{
           if (between2Points(startX6, startY6, xcenterF,ycenterF,xcenterF+30,ycenterF)){
              done6 = true;   
              drawCorrect = true;            
              g2d.fillRect(xcenterD, ycenterD, 30, 40);         
           }           
        }       
     }
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请根据要求先画出9节语文课的柱状图,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画出了语文课的柱状图，请继续画9节数学课的柱状图。你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经画出了语文、数学的柱状图，请继续画3节英语课的柱状图。你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经画出了语文、数学、英语的柱状图，请继续画2节体育课的柱状图。你画了" + (clickCount+1) + "次;";
                 }else{
                    if (done5 == false){
                       comment = "你已经画出了语文、数学、英语、体育的柱状图，请继续画3节音乐课的柱状图。你画了" + (clickCount+1) + "次;";
                    }else{
                       if (done6 == false){
                          comment = "你已经画出了语文、数学、英语、体育、音乐的柱状图，请继续画1节美术课的柱状图。你画了" + (clickCount+1) + "次;";
                       }else{
                          drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按照提示先后画出语文、数学、英语、体育、音乐、美术课的柱状图。";
        comment2 = "即画出9节语文、9节数学、3节英语、2节体育、3节音乐和1节美术课的条形统计图。"; 
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","语文","数学","英语","体育","音乐","美术",""};
     int intV = 0;
     for (int i=0; i<=120; i++){        
        if (i % 20 == 0){
           intV = i/20;           
           g2d.drawString(strs[intV], 100+i*3-30, 520);
        }
     }
     gp.moveTo(100,500);
     gp.lineTo(550,500);
