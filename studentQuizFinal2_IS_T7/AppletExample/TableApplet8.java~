...
public class TableApplet8 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new Table8Panel());
   }
}

class Table8Panel extends JPanel
 implements ActionListener
{  
    private boolean DEBUG = false;
    private JLabel label;
    private JButton button;
    private String GETINPUT = "请在上面表格中空白处填入恰当的名字，然后点击此按钮完成填空，最后关闭此页面返回原页面完成填空，而后提交答案。";
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if (command.equals(GETINPUT)) {
          ...
          if (!class1Value.equals("")){
             done1 = true;
             if (!class2Value.equals("")){
                done2 = true;
                if (!class3Value.equals("")){
                   done3 = true;
                   if (!class4Value.equals("")){
                      done4 = true; 
                      if (!class5Value.equals("")){
                         done5 = true;
                         if (!class6Value.equals("")){
                            done6 = true;  
    ...
    class MyTableModel extends AbstractTableModel {
        private String[] columnNames = {"", "四（1）班", "四（2）班", "胜者"};
        private Object[][] data = {	    
            {"第一次", "小芳", "", ""},
	    {"第二次", "小兰", "", ""},
            {"第三次", "小丽", "", ""}
