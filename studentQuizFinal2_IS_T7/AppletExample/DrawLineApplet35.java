...
public class DrawLineApplet35 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine35());
     
   }  
}

class DrawLine35 extends JPanel implements MouseListener, ActionListener
{
  ...
  DrawLine35()
  {
    ...
    if (target.equals("期末平行四边形A")){
       ...
       xcenterO2 = 383;
       ycenterO2 = 168; 
    }else{//if target.equals("期末平行四边形B")
       xcenterA = 200;
       ycenterA = 100; 
       xcenterB = 50;
       ycenterB = 175; 
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done2){
           ...
           if (inLine(xcenterD,ycenterD,xcenterC,ycenterC,startX2,startY2) && perpendicular(startX2,startY2,xcenterA,ycenterA,xcenterD,ycenterD,xcenterC,ycenterC)){
              done2 = true;     
              drawCorrect = true;         
              gp.moveTo(xcenterA,ycenterA);
              gp.lineTo(xcenterO2,ycenterO2);             
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请点击边BC上的一点，作通过顶点A垂直于边BC的平行四边形的高,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你只画对了一条高,请继续点击边CD上的一点,作通过顶点A垂直于边CD的平行四边形的高,你画了" + (clickCount+1) + "次;";
           }else{                                    
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示先画通过顶点A垂直于边BC的平行四边形的高；";
        comment2 = "再画通过顶点A垂直于边CD的平行四边形的高。"; 
     }
     if (target.equals("平行四边形A")){
        g2d.drawString(comment, 50, 630);
        if (clickCount<0)
           g2d.drawString(comment2, 50, 650);
     }else{
