...
public class ChartApplet13 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new ChartLine13());
     
   }  
}

class ChartLine13 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterB = 260; // Center position of B
  private static int xcenterC = 300;
  private static int ycenterC = 180; // Center position of C  
  ...
  public void paint(Graphics g)
  {
    ...
    g2d.drawString("小敏踢毽子成绩统计图", 150, 50);   
  }
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        ...
        if (done3){
           g2d.fillRect(xcenterC, ycenterC, 40, 320);         
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请根据要求先画出小敏第一次踢毽子18下的柱状图,你画了" + (clickCount+1) + "次"; 
        }else{
           if (done2==false){
              comment = "你已经画出了小敏第一次踢毽子的柱状图，请继续画小敏第二次踢毽子12下的柱状图。你画了" + (clickCount+1) + "次";
           }else{
              if (done3==false){
                 comment = "你已经画出了小敏第一次、第二次踢毽子的柱状图，请继续画小敏第三次踢毽子16下的柱状图。你画了" + (clickCount+1) + "次";
              }else{
                 if (done4 == false){
                    comment = "你已经画出了小敏第一次、第二次、第三次踢毽子的柱状图，请继续画小敏第四次踢毽子20下的柱状图。你画了" + (clickCount+1) + "次";
                 }else{
                    drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按照提示先后画出小敏第一次、第二次、第三次和第四次踢毽子的柱状图。";
        comment2 = "即画出第一次踢毽子18下、第二次踢毽子12下、第三次踢毽子16下和第四次踢毽子20下的条形统计图。"; 
     }
     g2d.drawString(comment, 50, 590);
     if (clickCount<0)
        g2d.drawString(comment2, 50, 610);
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","第","","第","","第","","第","",""};
     String[] strs2 = new String[]{"","一","","二","","三","","四","",""};
     String[] strs3 = new String[]{"","次","","次","","次","","次","",""};
     int intV = 0;
     for (int i=0; i<100; i++){        
        if (i % 10 == 0){
           intV = i/10;           
           g2d.drawString(strs[intV], 100+i*4, 520);
           g2d.drawString(strs2[intV], 100+i*4, 520+20);
           g2d.drawString(strs3[intV], 100+i*4, 520+40); 
        } 
     }
     gp.moveTo(100,500);
     gp.lineTo(540,500);
