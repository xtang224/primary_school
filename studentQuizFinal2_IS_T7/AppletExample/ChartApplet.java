...
{  
   ...
   {  
      ...
      contentPane.add(new ChartLine());
     
   }  
}

class ChartLine extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */    
     ...
     if (clickCount>=0){        
        ...
        if (done2){
           g2d.fillRect(xcenterB, ycenterB, 40, 400);            
        }else{
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请根据要求先画出6个西瓜的柱状图,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你已经画出了西瓜的柱状图，请继续画10个苹果的柱状图。你画了" + (clickCount+1) + "次;";
           }else{
              if (done3==false){
                 comment = "你已经画出了西瓜、苹果的柱状图，请继续画7个桃子的柱状图。你画了" + (clickCount+1) + "次;";
              }else{
                 if (done4 == false){
                    comment = "你已经画出了西瓜、苹果、桃子的柱状图，请继续画5个梨子的柱状图。你画了" + (clickCount+1) + "次;";
                 }else{
                    drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按照提示先后画出西瓜、苹果、桃子、梨子的柱状图。";
        comment2 = "即画出6个西瓜、10个苹果、7个桃子和5个梨子的条形统计图。"; 
     }
  ...
  public void drawRuler(Graphics2D g2d){
     ...
     GeneralPath gp = new GeneralPath();
     String[] strs = new String[]{"","西瓜","苹果","桃子","梨",""};
     int intV = 0;
     for (int i=0; i<100; i++){        
