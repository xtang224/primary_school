...
public class DrawLineApplet43 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine43());
     
   }  
}

class DrawLine43 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (target.equals("平行四边形高"))     
        g2d.drawString("底", (xcenterC+xcenterD)/2+20, (ycenterC+ycenterD)/2+20); 
     else if (target.equals("梯形高"))
        g2d.drawString("底", (xcenterC+xcenterB)/2+20, (ycenterC+ycenterB)/2+20); 
     else 
        g2d.drawString("底", (xcenterC+xcenterB)/2-20, (ycenterC+ycenterB)/2-20);
     g2d.draw(gp);
     ...
     if (clickCount>=0){
        if (done1==false){  
           if (target.equals("平行四边形高"))
              comment = "你还没有画对,请画出过顶点A垂直于对边的平行四边形的高,你画了" + (clickCount+1) + "次;";            
           else if (target.equals("梯形高"))
              comment = "你还没有画对,请画出过顶点A垂直于对边的梯形的高,你画了" + (clickCount+1) + "次;"; 
           else
              comment = "你还没有画对,请画出过顶点A垂直于对边的三角形的高,你画了" + (clickCount+1) + "次;"; 
        }else{
           drawCorrect = true;
        ...
        if (target.equals("平行四边形高")){      
           comment = "你还没有开始画：请过顶点A垂直于对边的平行四边形的高"; 
           comment2 = "";
        }else if (target.equals("梯形高")) { 
           comment = "你还没有开始画：请过顶点A垂直于对边的梯形的高"; 
           comment2 = "";
        }else{
           comment = "你还没有开始画：请过顶点A垂直于对边的三角形的高"; 
           comment2 = "";
        }
     }
