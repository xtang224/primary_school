...
public class DrawLineApplet49 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine49());
     
   }  
}

class DrawLine49 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterE = 350;
  private static int ycenterE = 150; // Center position of E 
  private static int xcenterO = 167;
  private static int ycenterO = 300; // Center position of O
  private static int xcenterO2 = 217;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请先点击BC上的一点，和D连线，划分出一个梯形,你画了" + (clickCount+1) + "次;"; 
           comment2 = "";
        }else{
           if (done2==false){              
              comment = "你已经连线划分出一个梯形，请继续点击AD上的一点，和你前面找到的点连线，完成划分。你画了" + (clickCount+1) + "次;";  
              comment2 = "";              
           }else{
              drawCorrect = true;
              comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交"; 
              comment2 = "说明：这样我们将这一图形分成了三角形、梯形和平行四边形;";                          
           }        
        }       
     }else{      
        comment = "你还没有开始画：请先在BC上找一点，和D连线，划分出一个梯形；";
        comment2 = "然后，再在AD上找一点，和第一步找到的点连线。"; 
        comment3 = "这两道线将这一图形分成三角形、梯形和平行四边形。";   
     }
