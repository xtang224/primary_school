...
public class GameApplet3 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new Game3Panel());
   }
}

class Game3Panel extends JPanel
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if (command.equals("点击此按钮提交你的对策")){
          ...
          if (gameSum < gameCritical){
             if (gameCritical-gameSum>=1 && gameCritical-gameSum<=4){
                ...
                if (Math.random()<0.9)
                   opponentInput += 1 + ";";
                else if (Math.random()>=0.92 && Math.random()<0.94) 
                   opponentInput += 2 + ";";
                else if (Math.random()>=0.94 && Math.random()<0.96) 
                   opponentInput += 3 + ";";
                else
                   opponentInput += 4 + ";";                
             }  
             ...
             label2.setText("游戏结束，所有的数加起来的和不能超过40。请点击最下面一个按钮完成这道题。");
             done = true;
             win = false;
             myTable.setValueAt(new String("对手"),3,3);    
             return; 
