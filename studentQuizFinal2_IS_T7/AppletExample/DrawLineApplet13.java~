...
public class DrawLineApplet13 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine13());
     
   }  
}

class DrawLine13 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterA = 100+40;
  private static int ycenterA = 500-40; // Center position of A
  private static int xcenterB = 100+160+40;
  private static int ycenterB = 500-40; // Center position of B
  ...
  DrawLine13()
  {
    ...
    if (target.equals("长方形A")){
       ...
       xcenterD = 100+120+40;
       ycenterD = 500-40-120; 
    }else{//if target.equals("60度平行四边形")
       xcenterB = 100+120+40;
       ycenterB = 500-40; 
       xcenterC = 180;
       ycenterC = 391; 
  ...
  public void drawArc(Graphics2D g2d){
     ...
     */
     ...
     if (clickCount>=0){
        if (done1==false){  
           if (target.equals("长方形A"))         
              comment = "你还没有画对,请继续点击画以A对端点的4厘米的长方形的长,你画了" + (clickCount+1) + "次;"; 
           else if (target.equals("长方形B"))
              comment = "你还没有画对,请继续点击画以A对端点的3厘米的长方形的长,你画了" + (clickCount+1) + "次;";
           else if (target.equals("正方形"))
              comment = "你还没有画对,请继续点击画以A对端点的3厘米的正方形的横边,你画了" + (clickCount+1) + "次;"; 
           else
              comment = "你还没有画对,请继续点击画以A对端点的3厘米的平行四边形的横边,你画了" + (clickCount+1) + "次;";
        }else{
           if (done2==false){
              if (target.equals("长方形A"))
                 comment = "你只画对了长方形的长,请继续点击画以A为端点的2厘米的长方形的宽,你画了" + (clickCount+1) + "次;";
              else if (target.equals("长方形B"))
                 comment = "你只画对了长方形的长,请继续点击画以A为端点的2厘米的长方形的宽,你画了" + (clickCount+1) + "次;";
              else if (target.equals("正方形"))
                 comment = "你只画对了正方形的横边,请继续点击画以A为端点的3厘米的正方形的竖边,你画了" + (clickCount+1) + "次;";
              else
                 comment = "你只画对了平行四边形的横边,请继续点击画以A为端点的2厘米的另一条边，且角度为60度,你画了" + (clickCount+1) + "次;";
           }else{                        
              drawCorrect = true;
        ...
        if (target.equals("长方形A"))      
           comment = "你还没有开始画：请按提示以点A为顶点，先画4厘米的长，再画2厘米的宽，完成这个长方形"; 
        else if (target.equals("长方形B"))
           comment = "你还没有开始画：请按提示以点A为顶点，先画3厘米的长，再画2厘米的宽，完成这个长方形"; 
        else if (target.equals("正方形"))
           comment = "你还没有开始画：请按提示以点A为顶点，先画3厘米的横边，再画3厘米的竖边，完成这个正方形";
        else
           comment = "你还没有开始画：请按提示以点A为顶点，先画3厘米的横边，再画角度为60度的2厘米的另一条边，完成这个平行四边形";
     }
     g2d.drawString(comment, 0, 550);
