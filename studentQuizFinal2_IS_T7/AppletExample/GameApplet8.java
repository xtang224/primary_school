...
public class GameApplet8 extends JApplet
{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new Game8Panel());
   }
}

class Game8Panel extends JPanel
...
{  
    ...
    private String problemHint = "你和对手轮流报数，每次只能报1,2或3，把两人报的所有数加起来，谁报数后和是15，谁就获胜";
    ...
    private int count = 0;
    private int maxGameSize = 4;
    private int gameTotal = 15;
    private int gameCritical = 7;
    ...
    public Game8Panel() {
        ...
        button2 = new JButton("点击此按钮看对手的对策");
        button2.setActionCommand("点击此按钮看对手的对策");
        button2.addActionListener(this);

        button3 = new JButton(GETINPUT);
        button3.setActionCommand(GETINPUT);
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if (command.equals("点击此按钮提交你的对策")){
          ...
          for (int i=0; i<maxGameSize; i++){
             ...
             if (tmpInteger.intValue()!=0){
                if (tmpInteger.intValue()<minInput || tmpInteger.intValue()>maxInput){
                   label2.setText("你只能填" + minInput + "-" + maxInput + ",请重新提交你的对策");
                   return;
                }
                label2.setText("你提交的对策是1,2或3，符合要求");
                yourInput += tmpInteger + ";";
             }
          }
          setYourInput();
       }else if (command.equals("点击此按钮看对手的对策")){
          getYourInput();
          getOpponentInput();
          System.out.println("yourInput =" + yourInput);  
          System.out.println("opponentInput =" + opponentInput);  
          ...
          if (gameSum < gameCritical){
             if (gameCritical-gameSum>=1 && gameCritical-gameSum<=2){
                opponentInput += (gameCritical-gameSum) + ";";
                if (gameCritical==gameTotal){
             ...
             while (gameCritical <= gameSum)
                gameCritical += step;
             System.out.println("in gameSum<gameTotal, gameCritical =" + gameCritical);
             if (gameCritical >= gameTotal){
                gameCritical = gameTotal;
                isEnd = true;
             }
             if (gameCritical-gameSum>=minInput && gameCritical-gameSum<=maxInput){
                opponentInput += (gameCritical-gameSum) + ";";
                if (isEnd){
                   label2.setText("游戏结束，请点击最下面一个按钮完成这道题。");
                   done = true;
             ...
             label2.setText("游戏结束，所有的数加起来的和不能超过"+ gameTotal + "。请点击最下面一个按钮完成这道题。");
             done = true;
             win = false;
             myTable.setValueAt(new String("对手"),3,3);    
             return; 
          ...
          for (int i=0; i<strs.length; i++){
             myTable.setValueAt(new Integer(strs[i].trim()),i,1);
          }
          System.out.println("opponentInput =" + opponentInput); 
          strs2 = opponentInput.split(";");
          for (int i=0; i<strs2.length; i++){
             myTable.setValueAt(new Integer(strs2[i].trim()),i,2);
          }
          setOpponentInput();
