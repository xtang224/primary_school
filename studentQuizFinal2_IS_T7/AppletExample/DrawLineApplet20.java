...
public class DrawLineApplet20 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine20());     
   }  
}

class DrawLine20 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int ycenterB = 250; // Center position of B
  private static int xcenterC = 350;
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请画通过顶点A的一条线段,将这个梯形分成一个平行四边形和一个三角形,你画了" + (clickCount+1) + "次;"; 
        }else{                                            
           drawCorrect = true;
           comment = "你完全画对了,你画了" + (clickCount+1) + "次;请返回原答题页面点击按钮完成填空，然后提交";              
        }       
     }else{   
        comment = "你还没有开始画，请用鼠标点击画线：请按提示画通过顶点A的一条线段；";
        comment2 = "这条线段将这个梯形分成一个平行四边形和一个三角形。"; 
     }
