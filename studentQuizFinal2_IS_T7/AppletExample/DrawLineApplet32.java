...
public class DrawLineApplet32 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine32());     
   }  
}

class DrawLine32 extends JPanel implements MouseListener, ActionListener
{
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        if (done1==false){  
           comment = "你还没有画对,请点击边AB上的一点，作通过顶点C垂直于边AB的平行四边形的高,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你只画对了一条高,请继续点击边BC上的一点,作通过顶点A垂直于边BC的平行四边形的高,你画了" + (clickCount+1) + "次;";
           }else{                                    
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示先画通过顶点C垂直于边AB的平行四边形的高；";
        comment2 = "再画通过顶点A垂直于边BC的平行四边形的高。"; 
     }
     if (target.equals("平行四边形A")){
