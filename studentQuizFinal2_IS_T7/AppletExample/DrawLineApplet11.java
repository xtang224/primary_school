...
public class DrawLineApplet11 extends JApplet{  
   public void init()
   {  
      Container contentPane = getContentPane();
      contentPane.add(new DrawLine11());     
   }  
}

class DrawLine11 extends JPanel implements MouseListener, ActionListener
{
  ...
  private static int xcenterB = 400;
  ...
  private static int xcenterO = 400;
  private static int ycenterO = 300; // Center position of O
  private static int xcenterO2 = 307;
  private static int ycenterO2 = 162;
  private static int xcenterO3 = 285;
  private static int ycenterO3 = 424; 
  ...
  public void drawArc(Graphics2D g2d){
     ...
     if (clickCount>=0){
        ...
        if (done1){           
           ...
           if (perpendicular(startX, startY, xcenterO,ycenterO, xcenterA,ycenterA,xcenterB,ycenterB) && inLine(xcenterA,ycenterA,xcenterB,ycenterB,startX, startY)){
              done1 = true;
              gp.moveTo(xcenterO,ycenterO);
              gp.lineTo(xcenterO2,ycenterO2);
           }           
        ...
        if (done2){           
           ...
           if (perpendicular(startX2, startY2, xcenterO,ycenterO, xcenterA,ycenterA,xcenterC,ycenterC) && inLine(xcenterA,ycenterA,xcenterC,ycenterC,startX2, startY2)){
              done2 = true;
              drawCorrect = true;
              gp.moveTo(xcenterO,ycenterO);
              gp.lineTo(xcenterO3,ycenterO3);   
     ...
     if (clickCount>=0){
        if (done1==false){           
           comment = "你还没有画对,请继续点击画直线AB的垂线,你画了" + (clickCount+1) + "次;"; 
        }else{
           if (done2==false){
              comment = "你画对了一条线,请继续点击画直线AC的垂线,你画了" + (clickCount+1) + "次;"; 
           }else{              
              drawCorrect = true;
        ...
        comment = "你还没有开始画，请用鼠标点击画线：请按提示依次过点O画直线AB和直线AC的垂线"; 
     }
